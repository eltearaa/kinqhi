...
session-scoped state file under ``$KINQHI_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $KINQHI_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_kinqhi_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this kinqhi install, leave it first —
    ...
    env = os.environ.copy()
    env["KINQHI_MEET_URL"] = url
    env["KINQHI_MEET_OUT_DIR"] = str(out)
    env["KINQHI_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["KINQHI_MEET_HEADED"] = "1"
    if auth_state:
        env["KINQHI_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["KINQHI_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if KINQHI_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["KINQHI_MEET_MODE"] = mode
    if realtime_model:
        env["KINQHI_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["KINQHI_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["KINQHI_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["KINQHI_MEET_REALTIME_KEY"] = realtime_api_key
