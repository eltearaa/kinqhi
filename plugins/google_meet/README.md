...
Let the kinqhi agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where kinqhi runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `kinqhi meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `kinqhi meet` CLI |
...
| `cli.py` | `kinqhi meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$KINQHI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `kinqhi meet node {run,list,approve,remove,status,ping}` |
...
kinqhi meet install                                      # pip + Chromium
kinqhi meet setup                                        # preflight
kinqhi meet auth                                         # optional
kinqhi meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
kinqhi meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.kinqhi/.env
kinqhi meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
kinqhi meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
kinqhi meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.kinqhi/.env
kinqhi meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, kinqhi will **not** switch your system audio input automatically — the
...
kinqhi plugins enable google_meet
kinqhi meet node run --display-name my-mac --host 0.0.0.0 --port 18789
