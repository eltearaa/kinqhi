...
```bash
kinqhi memory setup    # select "holographic"
```

Or manually:
```bash
kinqhi config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.kinqhi-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KINQHI_HOME/memory_store.db` | SQLite database path |
