...
Auto-tracks and cleans up ephemeral files created during Kinqhi
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$KINQHI_HOME` and `/tmp/kinqhi-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KINQHI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under KINQHI_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `KINQHI_HOME` or `/tmp/kinqhi-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$KINQHI_HOME/disk-cleanup/` is itself excluded
- `$KINQHI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
