"""disk_cleanup — ephemeral file cleanup for Kinqhi.
...
Rules:
  ...
  - empty dirs    → always delete (under KINQHI_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KINQHI_HOME and /tmp/kinqhi-*
Never touches: ~/.kinqhi/logs/ or any system directory.
"""
...
try:
    ...
    def get_kinqhi_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KINQHI_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".kinqhi").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KINQHI_HOME/logs/``."""
    return get_kinqhi_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KINQHI_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KINQHI_HOME or ``/tmp/kinqhi-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    kinqhi_home = get_kinqhi_home()
    try:
        path.resolve().relative_to(kinqhi_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/kinqhi-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("kinqhi-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "kinqhi", "backups", "profiles", ".worktrees",
})
...
# Paths under $KINQHI_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so KINQHI_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        kinqhi_home = get_kinqhi_home()
        for parent in ("cron", "cronjobs"):
            base = kinqhi_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KINQHI_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KINQHI_HOME, but never recurse into known
    # durable state trees.  Some installs place the Kinqhi checkout, venv,
    # and desktop build under KINQHI_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    kinqhi_home = get_kinqhi_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in kinqhi_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    kinqhi_home = get_kinqhi_home()
    try:
        rel = path.resolve().relative_to(kinqhi_home)
        ...
        # Path isn't under KINQHI_HOME (e.g. /tmp/kinqhi-*) — fall through.
        pass
