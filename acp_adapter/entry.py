"""CLI entry point for the kinqhi ACP adapter.

Loads environment variables from ``~/.kinqhi/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from KINQHI_HOME (default ``~/.kinqhi``)."""
    from kinqhi_cli.env_loader import load_kinqhi_dotenv

    kinqhi_home = get_kinqhi_home()
    loaded = load_kinqhi_dotenv(kinqhi_home=kinqhi_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", kinqhi_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="kinqhi-acp",
        description="Run Kinqhi as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Kinqhi version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Kinqhi provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.kinqhi/node/ "
...
def _print_version() -> None:
    from kinqhi_cli import __version__ as hermes_version
...
def _run_check() -> None:
    ...
    from acp_adapter.server import KinqhiACPAgent  # noqa: F401

    print("Kinqhi ACP check OK")


def _run_setup() -> None:
    from kinqhi_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "kinqhi", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``kinqhi postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting kinqhi ACP adapter")
    ...
    import acp
    from .server import KinqhiACPAgent
