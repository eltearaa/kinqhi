"""ACP agent server — exposes Kinqhi via the Agent Client Protocol."""
...
try:
    from kinqhi_cli import __version__ as KINQHI_VERSION
except Exception:
    KINQHI_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Kinqhi running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Kinqhi cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Kinqhi/OpenAI-compatible user content payload."""
...
class KinqhiACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Kinqhi AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Kinqhi version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Kinqhi maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from kinqhi_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from kinqhi_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Kinqhi estimates ``used`` from the same
    ...
    def _provenance_meta(
        ...
        """Best-effort ``_meta.kinqhi.sessionProvenance`` for an ACP session."""
        try:
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Kinqhi changes it.

        When the internal Kinqhi head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hermes_session_id`` so the
        attached ``_meta.kinqhi.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # kinqhi_state.py schema — only started_at/ended_at). Use "now" as
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["kinqhi-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kinqhi", version=KINQHI_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Kinqhi' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Kinqhi setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Kinqhi
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Kinqhi on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Kinqhi' local kawaii waiting/status
        ...
        # not the executor's. Also set KINQHI_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue KINQHI_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind KINQHI_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("KINQHI_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("KINQHI_SESSION_ID")
            os.environ["KINQHI_SESSION_ID"] = session_id
            try:
                ...
                # Restore KINQHI_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("KINQHI_INTERACTIVE", None)
                else:
                    os.environ["KINQHI_INTERACTIVE"] = previous_interactive
                # Restore KINQHI_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("KINQHI_SESSION_ID", None)
                else:
                    os.environ["KINQHI_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Kinqhi DB session id before the turn so we
            ...
            # stomp on each other's ContextVar writes (KINQHI_SESSION_KEY in
        ...
        # _meta.kinqhi.sessionProvenance so ACP clients can render the boundary
        ...
        # Kinqhi' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Kinqhi v{KINQHI_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Kinqhi has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
