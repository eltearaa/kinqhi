"""ACP auth helpers — detect and advertise Kinqhi authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "kinqhi-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Kinqhi runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Kinqhi can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Kinqhi.
    ...
    not have Kinqhi provider credentials configured yet, so Kinqhi always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Kinqhi using the currently configured "
