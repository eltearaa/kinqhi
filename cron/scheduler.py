...
Uses a file-based lock (~/.kinqhi/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `kinqhi update` reloads
# the module) fail with ModuleNotFoundError for kinqhi_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from kinqhi_constants import get_kinqhi_home
from kinqhi_cli._subprocess_compat import windows_hide_flags
from kinqhi_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``kinqhi tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from kinqhi_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_kinqhi_home: Path | None = None


def _get_kinqhi_home() -> Path:
    """Resolve Kinqhi home dynamically while preserving test monkeypatch hooks."""
    return _kinqhi_home or get_kinqhi_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    kinqhi_home = _get_kinqhi_home()
    lock_dir = kinqhi_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("KINQHI_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid KINQHI_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within KINQHI_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Kinqhi-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against KINQHI_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_kinqhi_home() / "scripts"
    ...
    # escape — scripts MUST reside within KINQHI_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    os.environ["KINQHI_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed KINQHI_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads KINQHI_SESSION_PLATFORM
    #     and KINQHI_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via KINQHI_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     KINQHI_SESSION_PLATFORM == "telegram".
    ...
    #     KINQHI_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read KINQHI_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the KINQHI_CRON_AUTO_DELIVER_* vars set
    # below, so clearing KINQHI_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "KINQHI_CRON_AUTO_DELIVER_PLATFORM",
        "KINQHI_CRON_AUTO_DELIVER_CHAT_ID",
        "KINQHI_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_kinqhi_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_kinqhi_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["KINQHI_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["KINQHI_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["KINQHI_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > KINQHI_MODEL env >
        ...
        model = job.get("model") or os.getenv("KINQHI_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_kinqhi_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("KINQHI_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_kinqhi_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject KINQHI_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # KINQHI_HOME. When a workdir is configured, also inject project
        ...
        # override via KINQHI_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("KINQHI_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid KINQHI_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        # Set KINQHI_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("KINQHI_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid KINQHI_CRON_MAX_PARALLEL value; defaulting to unbounded")
