...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``kinqhi://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``kinqhi://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"kinqhi://blueprint/{quote(blueprint.key)}{qs}"
