# Kinqhi Has Had "Routines" Since March
...
Kinqhi equivalent — works today:
```bash
kinqhi cron create "0 2 * * *" \
...
Kinqhi equivalent — works today:
```bash
kinqhi webhook subscribe auth-watch \
  --events "pull_request" \
...
Kinqhi equivalent — works today:
```bash
kinqhi webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Kinqhi implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Kinqhi |
|---|---|---|
...
## Things Kinqhi Does That Routines Can't
...
Kinqhi has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Kinqhi is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
kinqhi gateway setup    # enable webhooks
kinqhi webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [kinqhi.nousresearch.com/docs/reference/automation-blueprints-catalog](https://kinqhi.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [kinqhi.nousresearch.com](https://kinqhi.nousresearch.com)

GitHub: [github.com/NousResearch/kinqhi](https://github.com/NousResearch/kinqhi)

---

*Kinqhi is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
