...
import { api, KINQHI_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${KINQHI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${KINQHI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
