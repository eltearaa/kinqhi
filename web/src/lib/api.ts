...
// injects ``window.__KINQHI_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__KINQHI_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const KINQHI_BASE_PATH = readBasePath();
const BASE = KINQHI_BASE_PATH;
...
declare global {
  interface Window {
    __KINQHI_SESSION_TOKEN__?: string;
    __KINQHI_BASE_PATH__?: string;
    ...
    __KINQHI_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Kinqhi-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__KINQHI_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``kinqhi update``, ``kinqhi gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__KINQHI_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__KINQHI_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__KINQHI_SESSION_TOKEN__`` is valid, so the next
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__KINQHI_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Kinqhi dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``kinqhi_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__KINQHI_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Kinqhi-Session-Token`` header.
 ...
 * ``window.__KINQHI_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``kinqhi update``. */
