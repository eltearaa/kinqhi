"""Shared fixtures for the kinqhi test suite.
...
2. **Isolated KINQHI_HOME.** KINQHI_HOME points to a per-test tempdir so
   code reading ``~/.kinqhi/*`` via ``get_kinqhi_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".hermes"`` instead of the canonical
   ``get_kinqhi_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No KINQHI_SESSION_* inheritance** — the agent's current gateway
...
# KINQHI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_KINQHI_BEHAVIORAL_VARS = frozenset({
    "KINQHI_YOLO_MODE",
    "KINQHI_INTERACTIVE",
    "KINQHI_QUIET",
    "KINQHI_TOOL_PROGRESS",
    "KINQHI_TOOL_PROGRESS_MODE",
    "KINQHI_MAX_ITERATIONS",
    "KINQHI_SESSION_PLATFORM",
    "KINQHI_SESSION_CHAT_ID",
    "KINQHI_SESSION_CHAT_NAME",
    "KINQHI_SESSION_THREAD_ID",
    "KINQHI_SESSION_SOURCE",
    "KINQHI_SESSION_KEY",
    "KINQHI_GATEWAY_SESSION",
    "KINQHI_CRON_SESSION",
    "_KINQHI_GATEWAY",
    "KINQHI_PLATFORM",
    "KINQHI_MODEL",
    "KINQHI_INFERENCE_MODEL",
    "KINQHI_INFERENCE_PROVIDER",
    "KINQHI_TUI_PROVIDER",
    "KINQHI_MANAGED",
    "KINQHI_MANAGED_DIR",
    "KINQHI_DEV",
    ...
    "KINQHI_TIMEZONE",
    "KINQHI_REDACT_SECRETS",
    "KINQHI_BACKGROUND_NOTIFICATIONS",
    "KINQHI_EXEC_ASK",
    "KINQHI_HOME_MODE",
    "KINQHI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.kinqhi/kanban.db instead of the per-test KINQHI_HOME.
    "KINQHI_KANBAN_DB",
    "KINQHI_KANBAN_BOARD",
    "KINQHI_KANBAN_HOME",
    "KINQHI_KANBAN_WORKSPACES_ROOT",
    "KINQHI_KANBAN_LOGS_ROOT",
    "KINQHI_KANBAN_TASK",
    "KINQHI_KANBAN_WORKSPACE",
    "KINQHI_KANBAN_RUN_ID",
    "KINQHI_KANBAN_CLAIM_LOCK",
    "KINQHI_KANBAN_DISPATCH_IN_GATEWAY",
    "KINQHI_TENANT",
    ...
    "KINQHI_DASHBOARD_OAUTH_CLIENT_ID",
    "KINQHI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and KINQHI_HOME to per-test tempdirs so code that
    reads ``~/.kinqhi/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral KINQHI_* vars that could change test semantics.
    for name in _KINQHI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect KINQHI_HOME to a per-test tempdir. Code that reads
    #    ``~/.kinqhi/*`` via ``get_kinqhi_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.kinqhi/*`` via
    #    ``Path.home() / ".hermes"`` instead of ``get_kinqhi_home()``
    #    is a bug to fix at the callsite.
    fake_kinqhi_home = tmp_path / "hermes_test"
    fake_kinqhi_home.mkdir()
    (fake_kinqhi_home / "sessions").mkdir()
    (fake_kinqhi_home / "cron").mkdir()
    (fake_kinqhi_home / "memories").mkdir()
    (fake_kinqhi_home / "skills").mkdir()
    monkeypatch.setenv("KINQHI_HOME", str(fake_kinqhi_home))
    ...
    #    ~/.kinqhi/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import kinqhi_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_kinqhi_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal kinqhi config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``kinqhi-gateway`` process
...
# correlated with a ``tests/kinqhi_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> kinqhi-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart kinqhi-gateway"``,
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _KINQHI_TOKENS)
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m kinqhi_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live kinqhi-gateway systemd unit. Mock "
        ...
        # Block any subprocess that would run `kinqhi update` (or the
        # equivalent `python -m kinqhi_cli.main update`).  These commands
        ...
        if "update" in low and (
            # kinqhi update / kinqhi update --gateway / setsid bash -c ... kinqhi update
            ("hermes" in low and "update" in low.split())
            or
            # python -m kinqhi_cli.main update --gateway
            ("kinqhi_cli" in low and "update" in low.split())
            or
            # venv/bin/kinqhi update  (absolute path variant used in tests)
            (".venv/bin/hermes" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`kinqhi update` against the real checkout, fetching "
