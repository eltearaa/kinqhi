"""
Tests for timezone support (kinqhi_time module + integration points).
...
def _reset_kinqhi_time_cache():
    """Reset the kinqhi_time module cache (replacement for removed reset_cache)."""
    kinqhi_time._cached_tz = None
    kinqhi_time._cached_tz_name = None
    kinqhi_time._cache_resolved = False


# =========================================================================
# kinqhi_time.now() — core helper
# =========================================================================

class TestKinqhiTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_kinqhi_time_cache()

    def teardown_method(self):
        _reset_kinqhi_time_cache()
        os.environ.pop("KINQHI_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["KINQHI_TIMEZONE"] = "Asia/Kolkata"
        result = kinqhi_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["KINQHI_TIMEZONE"] = "UTC"
        result = kinqhi_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["KINQHI_TIMEZONE"] = "America/New_York"
        result = kinqhi_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["KINQHI_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="kinqhi_time"):
            result = kinqhi_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["KINQHI_TIMEZONE"] = "Europe/London"
        tz = kinqhi_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["KINQHI_TIMEZONE"] = "Not/A/Timezone"
        tz = kinqhi_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When KINQHI_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) KINQHI_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("KINQHI_TIMEZONE=" + os.environ.get("KINQHI_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "KINQHI_TIMEZONE=NOT_SET" in result["output"], (
            "KINQHI_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When KINQHI_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("KINQHI_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Kinqhi tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to Kinqhi tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Kinqhi tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Kinqhi tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Kinqhi is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Kinqhi tz
        ...
        # Use a Kinqhi timezone far behind UTC so that the numeric wall time
        ...
        os.environ["KINQHI_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_kinqhi_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Kinqhi timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["KINQHI_TIMEZONE"] = "US/Eastern"
        _reset_kinqhi_time_cache()
