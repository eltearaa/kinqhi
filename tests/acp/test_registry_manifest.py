"""Tests for ACP Registry metadata shipped with Kinqhi."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "kinqhi"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Kinqhi"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/kinqhi"
    assert data["website"].startswith("https://kinqhi.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"kinqhi[acp]=={data['version']}"
    assert uvx["args"] == ["kinqhi-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"kinqhi[acp]=={_pyproject_version()}"
    )
