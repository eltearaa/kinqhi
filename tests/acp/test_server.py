"""Tests for acp_adapter.server — KinqhiACPAgent ACP server."""
...
from acp_adapter.server import KinqhiACPAgent, KINQHI_VERSION
...
def agent(mock_manager):
    """KinqhiACPAgent backed by a mock session manager."""
    return KinqhiACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "kinqhi"
        assert resp.agent_info.version == KINQHI_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Kinqhi' interactive model/provider setup in a terminal. "
                    "Use this when Kinqhi has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Kinqhi provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = KinqhiACPAgent(session_manager=manager)

        with patch(
            "kinqhi_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "KinqhiACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("KinqhiACPAgent")
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "kinqhi_cli.models.parse_model_input",
...
class TestPrompt:
    ...
    async def test_prompt_propagates_hermes_session_id_env(self, agent, monkeypatch):
        ...
        via ``KINQHI_SESSION_ID`` so tools that want to stamp side-effects
        ...
        def mock_run(user_message, conversation_history=None, task_id=None, **kwargs):
            ...
            captured["env"] = os.environ.get("KINQHI_SESSION_ID")
        ...
        assert captured["env"] == new_resp.session_id, (
            "KINQHI_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("KINQHI_SESSION_ID") is None, (
            "KINQHI_SESSION_ID must be restored after the agent call so "
    ...
    async def test_prompt_restores_prior_hermes_session_id(self, agent, monkeypatch):
        ...
        def mock_run(*args, **kwargs):
            captured["inner"] = os.environ.get("KINQHI_SESSION_ID")
            return {"final_response": "ok", "messages": []}
        ...
        assert os.environ.get("KINQHI_SESSION_ID") == "outer-sess"
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert KINQHI_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["kinqhi-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["kinqhi-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["kinqhi-acp", "mcp-srv"]
