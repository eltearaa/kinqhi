...
AlexKucera reported on Discord (2026-04-16) that after ``kinqhi update`` pulled
...
Opt-in — not part of default CI:
    KINQHI_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.kinqhi/.env).
"""
...
# Load ~/.kinqhi/.env so live runs pick up OPENROUTER_API_KEY without
...
LIVE = os.environ.get("KINQHI_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set KINQHI_LIVE_TESTS=1"),
