...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    kinqhi_state = ModuleType("kinqhi_state")
    kinqhi_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "kinqhi_state", kinqhi_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Kinqhi"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Kinqhi"
