"""Tests for the decomposer module + `kinqhi kanban decompose` CLI surface.
...
def _patch_list_profiles(names: list[str]):
    ...
    return [
        patch("kinqhi_cli.profiles.list_profiles", return_value=fake_profiles),
        patch("kinqhi_cli.profiles.profile_exists", side_effect=lambda x: x in names),
        patch("kinqhi_cli.profiles.get_active_profile_name", return_value=names[0] if names else "default"),
    ]
...
def test_decompose_fanout_false_assigns_default_when_unassigned(kanban_home):
    ...
    try:
        with _patch_aux_client(llm_payload), _patch_extra_body(), patch(
            "kinqhi_cli.kanban_decompose._load_config",
