...
_VARS = ("KINQHI_SAFE_MODE", "KINQHI_IGNORE_USER_CONFIG", "KINQHI_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in kinqhi_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("KINQHI_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under KINQHI_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("KINQHI_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("KINQHI_SAFE_MODE", raising=False)
        from kinqhi_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under KINQHI_SAFE_MODE=1."""
