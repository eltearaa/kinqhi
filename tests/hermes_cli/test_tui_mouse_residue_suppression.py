...
where stdin is still in cooked + echo mode. If a previous Kinqhi session
...
from kinqhi_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        ...
        monkeypatch.delenv("KINQHI_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("KINQHI_TUI", raising=False)
        monkeypatch.setenv("KINQHI_TUI_NO_EARLY_DISABLE", "1")
