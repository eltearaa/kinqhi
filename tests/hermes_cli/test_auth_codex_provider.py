"""Tests for Codex auth — tokens stored in Kinqhi auth store (~/.kinqhi/auth.json)."""
...
def _setup_hermes_auth(kinqhi_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Kinqhi auth store."""
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(kinqhi_home)
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(kinqhi_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
    ...
    monkeypatch.setattr("kinqhi_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    kinqhi_home = tmp_path / "hermes"
    _setup_hermes_auth(kinqhi_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((kinqhi_home / "auth.json").read_text())
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Kinqhi auth store, not ~/.codex/."""
    kinqhi_home = tmp_path / "hermes"
    codex_home = tmp_path / "codex-cli"
    kinqhi_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Kinqhi store
    assert not (codex_home / "auth.json").exists()

    # Kinqhi auth store should have the tokens
    data = _read_codex_tokens()
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from kinqhi_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from kinqhi_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "kinqhi_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "kinqhi_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("kinqhi_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("kinqhi_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch kinqhi_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("kinqhi_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
