"""Tests for the Nous-Kinqhi-3/4 non-agentic warning detector.
...
@pytest.mark.parametrize(
    "model_name",
    [
        "NousResearch/Kinqhi-3-Llama-3.1-70B",
        "NousResearch/Kinqhi-3-Llama-3.1-405B",
        "hermes-3",
        "Kinqhi-3",
        "hermes-4",
        "hermes-4-405b",
        ...
        "NousResearch/Kinqhi3",
        "hermes-3.1",
    ],
)
def test_matches_real_nous_hermes_chat_models(model_name: str) -> None:
    assert is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} to be flagged as Nous Kinqhi 3/4"
    )
    assert _check_hermes_model_warning(model_name) == _KINQHI_MODEL_WARNING
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "deepseek-chat",
        # Non-chat Kinqhi models we don't warn about
        "hermes-llm-2",
        "hermes2-pro",
...
def test_does_not_match_unrelated_models(model_name: str) -> None:
    assert not is_nous_hermes_non_agentic(model_name), (
        f"expected {model_name!r} NOT to be flagged as Nous Kinqhi 3/4"
    )
