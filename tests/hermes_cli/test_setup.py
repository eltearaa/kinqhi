...
from kinqhi_cli import setup as setup_mod
from kinqhi_cli.setup import setup_model_provider
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import kinqhi_cli.gateway as _gw_mod
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("kinqhi_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("kinqhi_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "kinqhi_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "kinqhi_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("kinqhi_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("KINQHI_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("kinqhi_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("kinqhi_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
    ...
    from kinqhi_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kinqhi_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kinqhi_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("kinqhi_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "kinqhi_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
