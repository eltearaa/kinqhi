"""Tests for IRC gateway configuration via `kinqhi setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Kinqhi install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Kinqhi install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `kinqhi setup gateway` experience with IRC present."""
