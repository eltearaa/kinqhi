...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh KINQHI_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("KINQHI_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("kinqhi_cli") or mod.startswith("kinqhi_state") or mod == "kinqhi_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: kinqhi kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "kinqhi_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("kinqhi_cli.config.load_config", lambda: fake_config)
