...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("KINQHI_TUI", raising=False)
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("KINQHI_TUI", raising=False)
    monkeypatch.delenv("KINQHI_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("KINQHI_FAST_STARTUP_BANNER", raising=False)
    ...
    assert os.environ["KINQHI_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["KINQHI_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("KINQHI_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["hermes", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Kinqhi v" in out
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("KINQHI_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("KINQHI_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["hermes", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_kinqhi_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("KINQHI_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import kinqhi_cli.oneshot as oneshot_mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from kinqhi_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """hermes -z bypasses KinqhiCLI, but recall still needs SessionDB."""
    from kinqhi_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "kinqhi_state", mod("kinqhi_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "kinqhi_cli.config",
        mod("kinqhi_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "kinqhi_cli.models",
        mod("kinqhi_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "kinqhi_cli.runtime_provider",
        mod(
            "kinqhi_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "kinqhi_cli.tools_config",
        mod("kinqhi_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["KINQHI_TUI_ACTIVE_SESSION_FILE"])
    ...
    env = captured["env"]
    assert env["KINQHI_MODEL"] == "nous/hermes-test"
    assert env["KINQHI_INFERENCE_MODEL"] == "nous/hermes-test"
    assert env["KINQHI_TUI_PROVIDER"] == "nous"
    assert env["KINQHI_INFERENCE_PROVIDER"] == "nous"
    assert env["KINQHI_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["KINQHI_TUI_ACTIVE_SESSION_FILE"])
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_kinqhi_home
):
    captured = {}
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("kinqhi_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("KINQHI_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "KINQHI_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["KINQHI_TUI_RESUME"] == "20260518_000000_goodid"
...
def test_make_tui_argv_dev_prebuilds_hermes_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("KINQHI_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "kinqhi_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
