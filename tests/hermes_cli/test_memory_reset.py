"""Tests for the `kinqhi memory reset` CLI command.
...
- Profile-scoped reset (uses KINQHI_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KINQHI_HOME with memory files."""
    kinqhi_home = tmp_path / ".hermes"
    memories = kinqhi_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nKinqhi repo is at ~/.kinqhi/kinqhi\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `kinqhi memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        (kinqhi_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KINQHI_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_kinqhi_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
