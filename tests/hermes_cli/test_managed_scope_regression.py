...
def kinqhi_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KINQHI_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(kinqhi_home, monkeypatch):
    from kinqhi_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(kinqhi_home, monkeypatch):
    from kinqhi_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(kinqhi_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_hermes_dotenv(kinqhi_home=str(home))
