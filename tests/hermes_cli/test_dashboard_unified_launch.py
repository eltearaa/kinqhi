...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "kinqhi_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # KINQHI_HOME.  For a standard install (KINQHI_HOME unset) that root is
        # the platform-native default (~/.kinqhi), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from kinqhi_constants import get_default_hermes_root
        assert env.get("KINQHI_HOME") == str(get_default_hermes_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (KINQHI_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop KINQHI_HOME.

        Dropping it makes the child fall back to $HOME/.kinqhi
        ...
        """
        monkeypatch.setenv("KINQHI_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "kinqhi_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
        ...
        assert env.get("KINQHI_HOME") == "/opt/data"

    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (KINQHI_DESKTOP=1) must NOT
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "kinqhi_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        monkeypatch.delenv("KINQHI_WEB_DIST", raising=False)
        ...
        calls = []
        monkeypatch.setattr(
            "kinqhi_cli.mcp_startup.start_background_mcp_discovery",
            lambda **kwargs: calls.append(kwargs),
        )
        monkeypatch.setitem(
            sys.modules,
            "kinqhi_cli.web_server",
