...
When ``KINQHI_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Profile     -> tmp_path/.hermes/profiles/coder   (active, KINQHI_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Kinqhi is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from kinqhi_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    kinqhi_home = tmp_path / "classic"
    kinqhi_home.mkdir()
    monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))

    _write(kinqhi_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (KINQHI_HOME == global root), no fallback path runs.
    ...
    # to the KINQHI_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    kinqhi_home = tmp_path / "classic"
    ...
    _write(kinqhi_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from kinqhi_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: KINQHI_HOME is set to a custom path that is NOT under
    # ~/.kinqhi/profiles/ — get_default_hermes_root() returns KINQHI_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from kinqhi_cli.auth import read_credential_pool, write_credential_pool
