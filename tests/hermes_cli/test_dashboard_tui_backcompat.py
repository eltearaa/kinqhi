"""Regression test: `kinqhi dashboard --tui` must not hard-crash.

Older Kinqhi desktop app shells (<= 0.15.x) spawn the backend as::

    kinqhi dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Kinqhi couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real kinqhi_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "kinqhi_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
