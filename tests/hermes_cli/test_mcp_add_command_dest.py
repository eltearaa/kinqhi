"""Regression test: ``kinqhi mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``kinqhi_cli/_parser.py``).  The dispatcher in ``kinqhi_cli/main.py``
...
result, ``kinqhi mcp add foo --url ...`` silently launches chat instead
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`kinqhi mcp add foo --url ...` must keep args.command == "mcp".
