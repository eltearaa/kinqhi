...
import kinqhi_cli.doctor as doctor
import kinqhi_cli.gateway as gateway_cli
from kinqhi_cli import doctor as doctor_mod
from kinqhi_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `kinqhi doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "KINQHI_HOME", kinqhi_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("KINQHI_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("KINQHI_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_kinqhi_home(self, tmp_path, provider=""):
        """Create a minimal KINQHI_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_kinqhi_home(tmp_path, provider)
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from kinqhi_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("kinqhi_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "kinqhi_cli.doctor", raising=False)
        from kinqhi_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale KINQHI_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir(parents=True)
        (kinqhi_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"KINQHI_MAX_ITERATIONS={ghost}\n")
        (kinqhi_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "KINQHI_HOME", kinqhi_home)
        monkeypatch.setattr(doctor_mod, "get_kinqhi_home", lambda: kinqhi_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("KINQHI_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("KINQHI_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale KINQHI_MAX_ITERATIONS" in out
        env_after = (kinqhi_home / ".env").read_text(encoding="utf-8")
        assert "KINQHI_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`kinqhi doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Kinqhi install.
