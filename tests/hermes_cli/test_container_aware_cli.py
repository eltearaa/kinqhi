...
def container_env(tmp_path, monkeypatch):
    """Set up a fake KINQHI_HOME with .container-mode file."""
    kinqhi_home = tmp_path / ".hermes"
    kinqhi_home.mkdir()
    monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
    monkeypatch.delenv("KINQHI_DEV", raising=False)

    container_mode = kinqhi_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=kinqhi\n"
        "exec_user=hermes\n"
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("kinqhi_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "kinqhi"
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("kinqhi_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when KINQHI_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("KINQHI_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """KINQHI_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("KINQHI_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        kinqhi_home = Path(tmpdir) / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("kinqhi_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_kinqhi_home": lambda: kinqhi_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KINQHI_DEV", None)
            info = get_container_exec_info()
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from kinqhi_cli.main import _exec_in_container
