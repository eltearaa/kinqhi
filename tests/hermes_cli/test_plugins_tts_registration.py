...
``$KINQHI_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/kinqhi_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        kinqhi_home = Path(os.environ["KINQHI_HOME"])
        _write_plugin(
            kinqhi_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(kinqhi_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(kinqhi_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(kinqhi_home, "shadow-tts-plugin")
