"""Tests for `kinqhi memory setup [provider]` routing.
...
``kinqhi memory setup honcho``) without the interactive picker — which matters
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "kinqhi memory setup" in out
