"""Behavior tests for kinqhi_cli.inventory.
...
from kinqhi_cli.inventory import (
    ConfigContext,
...
def test_load_picker_context_full_dict():
    ...
    with patch("kinqhi_cli.config.load_config", return_value=cfg):
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("kinqhi_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "kinqhi_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("kinqhi_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # kinqhi model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `kinqhi model`")
        )
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("kinqhi_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from kinqhi_cli import model_switch

    with patch("kinqhi_cli.models.clear_provider_models_cache") as clear:
