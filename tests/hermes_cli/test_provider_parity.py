...
the SAME provider universe as ``kinqhi model`` (the CLI/TUI picker).
...
def test_every_hermes_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `kinqhi model` universe."""
    ...
    assert not missing, (
        "providers shown in `kinqhi model` but not configurable in the desktop "
