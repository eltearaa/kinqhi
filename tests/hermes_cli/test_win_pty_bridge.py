"""Unit tests for kinqhi_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/kinqhi_cli/test_pty_bridge.py.  Drives
...
``kinqhi_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from kinqhi_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from kinqhi_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from kinqhi_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from kinqhi_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('KINQHI_PTY_TEST=' + os.environ.get('KINQHI_PTY_TEST',''))",
            ],
            env={**os.environ, "KINQHI_PTY_TEST": "pty-env-works"},
        )
        try:
