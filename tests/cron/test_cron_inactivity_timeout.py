...
- Unlimited timeout (KINQHI_CRON_TIMEOUT=0)
- Backward compat: KINQHI_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """KINQHI_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """KINQHI_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("KINQHI_CRON_TIMEOUT", "1200")
        raw = os.getenv("KINQHI_CRON_TIMEOUT", "").strip()
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """KINQHI_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("KINQHI_CRON_TIMEOUT", "0")
        raw = os.getenv("KINQHI_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """KINQHI_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("KINQHI_CRON_TIMEOUT", "abc")
        raw = os.getenv("KINQHI_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """KINQHI_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("KINQHI_CRON_TIMEOUT", "")
        raw = os.getenv("KINQHI_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_kinqhi_time_importable(self):
        """kinqhi_time should be importable when cron.scheduler loads."""
    ...
    def test_kinqhi_constants_importable(self):
        """kinqhi_constants should be importable from cron context."""
        from kinqhi_constants import get_kinqhi_home
        assert callable(get_kinqhi_home)
