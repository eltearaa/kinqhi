...
class TestKinqhiConstantsFallback:
    """Tests for _kinqhi_home.py fallback when kinqhi_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_kinqhi_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _kinqhi_home.py with kinqhi_constants blocked."""
        monkeypatch.setitem(sys.modules, "kinqhi_constants", None)
        spec = importlib.util.spec_from_file_location("_kinqhi_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_kinqhi_home_env_var(self, monkeypatch, tmp_path):
        """When kinqhi_constants is missing, KINQHI_HOME comes from env var."""
        monkeypatch.setenv("KINQHI_HOME", str(tmp_path / "custom-hermes"))
        module = self._load_helper(monkeypatch)
        assert module.get_kinqhi_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When kinqhi_constants is missing and KINQHI_HOME unset, default to ~/.kinqhi."""
        monkeypatch.delenv("KINQHI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_kinqhi_home() == Path.home() / ".hermes"

    def test_fallback_ignores_empty_kinqhi_home(self, monkeypatch):
        """Empty/whitespace KINQHI_HOME is treated as unset."""
        monkeypatch.setenv("KINQHI_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_kinqhi_home() == Path.home() / ".hermes"

    def test_fallback_display_kinqhi_home_shortens_path(self, monkeypatch):
        """Fallback display_kinqhi_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("KINQHI_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_kinqhi_home() == "~/.kinqhi"

    def test_fallback_display_kinqhi_home_profile_path(self, monkeypatch):
        """Fallback display_kinqhi_home() handles profile paths under ~/."""
        monkeypatch.setenv("KINQHI_HOME", str(Path.home() / ".hermes/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_kinqhi_home() == "~/.kinqhi/profiles/coder"

    def test_fallback_display_kinqhi_home_custom_path(self, monkeypatch):
        """Fallback display_kinqhi_home() returns full path for non-home locations."""
        monkeypatch.setenv("KINQHI_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_kinqhi_home() == "/opt/hermes-custom"

    def test_delegates_to_kinqhi_constants_when_available(self):
        """When kinqhi_constants IS importable, _kinqhi_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_kinqhi_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import kinqhi_constants
        assert module.get_kinqhi_home is kinqhi_constants.get_kinqhi_home
        assert module.display_kinqhi_home is kinqhi_constants.display_kinqhi_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Kinqhi Docker image, whose venv is built with
