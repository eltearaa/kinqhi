...
def test_kinqhi_client_tag_includes_current_version():
    """The client tag must reflect kinqhi_cli.__version__ verbatim."""
    from kinqhi_cli import __version__
    from agent.portal_tags import kinqhi_client_tag

    assert kinqhi_client_tag() == f"client=kinqhi-client-v{__version__}"


def test_kinqhi_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import kinqhi_client_tag

    tag = kinqhi_client_tag()
    assert tag.startswith("client=kinqhi-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import kinqhi_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=kinqhi" in tags
    assert kinqhi_client_tag() in tags
    assert len(tags) == 2
