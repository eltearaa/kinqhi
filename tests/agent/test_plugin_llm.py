...
""",
        ...
        from kinqhi_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
        from kinqhi_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from kinqhi_cli.plugins import PluginContext, PluginManifest, PluginManager
