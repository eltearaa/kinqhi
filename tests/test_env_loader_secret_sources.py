"""Tests for the secret-source tracking in ``kinqhi_cli.env_loader``.

These cover the small public surface that lets `kinqhi model` / `kinqhi setup`
...
from kinqhi_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, kinqhi_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per KINQHI_HOME per process.
    """
    ...
    # cli.py, kinqhi_cli/main.py, run_agent.py, trajectory_compressor.py,
