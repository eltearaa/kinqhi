...
closure the PR changed, against a real temp ``KINQHI_HOME``.
"""
...
def _fake_switch_result():
    ...
    from kinqhi_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "kinqhi_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_kinqhi_home() / config.yaml`` — point it here.
    monkeypatch.setattr("kinqhi_constants.get_kinqhi_home", lambda: kinqhi_home)
    monkeypatch.setattr("kinqhi_cli.config.get_kinqhi_home", lambda: kinqhi_home)
    return cfg_path
