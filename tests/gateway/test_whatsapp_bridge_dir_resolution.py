...
bridge source into a writable KINQHI_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kinqhi_constants.get_kinqhi_home", lambda: kinqhi_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into KINQHI_HOME.
    assert not (kinqhi_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_kinqhi_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable KINQHI_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = kinqhi_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the KINQHI_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    kinqhi_home = tmp_path / "kinqhi_home"
    mirror = kinqhi_home / "scripts" / "whatsapp-bridge"
