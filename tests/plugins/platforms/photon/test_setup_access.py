...
channel at their DM, writing to the per-test ~/.kinqhi/.env (the hermetic
KINQHI_HOME fixture isolates this). It must fill only unset keys so a re-run
...
def test_setup_hint_uses_gateway_service_command(monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    ...
    assert "Start the gateway:  kinqhi gateway start" in out
