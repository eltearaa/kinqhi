...
def kinqhi_home(tmp_path, monkeypatch):
    """Point Kinqhi at an isolated home directory."""
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("KINQHI_HOME", str(home))
    # Some modules cache get_kinqhi_home; clear if needed.
    import kinqhi_constants
    if hasattr(kinqhi_constants, "_KINQHI_HOME_CACHE"):
        kinqhi_constants._KINQHI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(kinqhi_home, monkeypatch):
...
def test_install_bws_happy_path(kinqhi_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(kinqhi_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(kinqhi_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from kinqhi_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under kinqhi_home/cache/."""
    home = tmp_path / ".hermes"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
