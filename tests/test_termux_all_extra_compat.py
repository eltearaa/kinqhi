...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"kinqhi[termux]"' in text
    assert '"kinqhi[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"kinqhi[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
