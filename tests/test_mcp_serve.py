"""
Tests for mcp_serve — Kinqhi MCP server.
...
def _isolate_kinqhi_home(tmp_path, monkeypatch):
    """Redirect KINQHI_HOME to a temp directory."""
    monkeypatch.setenv("KINQHI_HOME", str(tmp_path))
    try:
        import kinqhi_constants
        monkeypatch.setattr(kinqhi_constants, "get_kinqhi_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking kinqhi_state schema."""
