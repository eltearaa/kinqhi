...
failed: Kinqhi is not logged into Nous Portal" on every message.
...
3. Files written under $KINQHI_HOME from a ``docker exec`` session land
   as hermes:hermes — the actual user-visible invariant.
4. The KINQHI_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `kinqhi config set` which writes config.yaml under KINQHI_HOME.
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """KINQHI_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "KINQHI_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
    ...
    assert r.stdout.strip() == "root:root", (
        f"With KINQHI_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``KINQHI_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``KINQHI_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"KINQHI_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    "Kinqhi is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `kinqhi config set`-style
    ...
    # under the shim's UID. We invoke `kinqhi config set` first to
    # provoke a write into KINQHI_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # KINQHI_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "hermes", sleep_container,
