...
Override the image with ``KINQHI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("KINQHI_TEST_IMAGE", "kinqhi-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``KINQHI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("KINQHI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
#
# Background: every Kinqhi runtime path inside the container drops to UID
