...
``$KINQHI_GIT_SHA`` build-arg to ``/opt/hermes/.hermes_build_sha`` and
``kinqhi_cli/build_info.py`` reads it as a fallback.
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``KINQHI_GIT_SHA``, dump must surface it.
