...
Before the s6 migration, ``docker run nousresearch/kinqhi gateway
...
crash and a supervised dashboard alongside (when ``KINQHI_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``KINQHI_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``KINQHI_S6_SUPERVISED_CHILD`` sentinel breaks the loop.
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``kinqhi gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "KINQHI_GATEWAY_NO_SUPERVISE=1",
    ...
    if inspect.stdout.strip() == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "KINQHI_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The KINQHI_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``kinqhi gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``kinqhi gateway
    ...
    # Count python processes running `kinqhi gateway run`. If the
    ...
    assert n <= 1, (
        f"expected at most one supervised python `kinqhi gateway run` "
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``KINQHI_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${KINQHI_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Kinqhi Gateway Starting…"
    ...
    assert "⚕" in combined or "Kinqhi Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "Kinqhi Gateway Starting" in file_contents, (
