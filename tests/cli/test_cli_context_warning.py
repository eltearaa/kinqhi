...
def _isolate(tmp_path, monkeypatch):
    """Isolate KINQHI_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal KinqhiCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = KinqhiCLI.__new__(KinqhiCLI)
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Kinqhi' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Kinqhi' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Kinqhi' minimum context length."""
