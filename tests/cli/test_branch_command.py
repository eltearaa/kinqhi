...
def session_db(tmp_path):
    ...
    os.environ["KINQHI_HOME"] = str(tmp_path / ".hermes")
...
def cli_instance(tmp_path, session_db):
    """Create a minimal KinqhiCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        KinqhiCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        KinqhiCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_branch_rotates_hermes_session_id_env_and_context(self, cli_instance, session_db):
        ...
        os.environ["KINQHI_SESSION_ID"] = old_session_id
        _VAR_MAP["KINQHI_SESSION_ID"].set(old_session_id)

        try:
            KinqhiCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["KINQHI_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("KINQHI_SESSION_ID") == cli_instance.session_id
        finally:
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from kinqhi_cli.commands import COMMAND_REGISTRY
