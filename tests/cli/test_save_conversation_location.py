...
The fix writes snapshots under ``~/.kinqhi/sessions/saved/`` and prints
...
def kinqhi_home(tmp_path, monkeypatch):
    ...
    # Clear any cached kinqhi_home computation
    import kinqhi_constants
    if hasattr(kinqhi_constants, "_kinqhi_home_cache"):
        kinqhi_constants._kinqhi_home_cache = None
    return home
...
def test_save_conversation_writes_under_kinqhi_home(kinqhi_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.kinqhi/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the KINQHI_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "kinqhi_constants"]:
    ...
    cli.KinqhiCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hermes_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.kinqhi/sessions/saved/
    saved_dir = kinqhi_home / "sessions" / "saved"
...
def test_save_conversation_empty_history_does_nothing(kinqhi_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "kinqhi_constants"]:
        sys.modules.pop(mod, None)
    import cli
