...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_kinqhi_home_override(self, tmp_path):
        """Auth lookup should read from KINQHI_HOME/auth.json, not ~/.kinqhi/auth.json."""
...
class TestBackendSelection:
    ...
    ``kinqhi tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "kinqhi_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
