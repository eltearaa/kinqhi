...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("kinqhi_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("kinqhi_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"KINQHI_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.kinqhi/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_kinqhi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $KINQHI_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_kinqhi_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$KINQHI_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestKinqhiConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.kinqhi/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.kinqhi/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.kinqhi/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.kinqhi/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.kinqhi/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.kinqhi/config.yaml")
        assert dangerous is True
        assert "kinqhi config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.kinqhi/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_kinqhi_home_config(self):
    ...
    def test_sed_in_place_absolute_kinqhi_home_env(self):
        env_path = get_kinqhi_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_kinqhi_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $KINQHI_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.kinqhi/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_kinqhi_home_config(self):
        config_path = get_kinqhi_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.kinqhi/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_kinqhi_home_env(self):
        env_path = get_kinqhi_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.kinqhi/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.kinqhi/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.kinqhi/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.kinqhi/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.kinqhi/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Kinqhi config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_kinqhi_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $KINQHI_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.kinqhi/config.yaml/.env must require approval — the
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.kinqhi/kinqhi && source venv/bin/activate && python -m kinqhi_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m kinqhi_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m kinqhi_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "kinqhi_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart kinqhi-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("KINQHI_GATEWAY_SESSION", "KINQHI_CRON_SESSION",
                      "KINQHI_YOLO_MODE",
                      "KINQHI_SESSION_KEY", "KINQHI_INTERACTIVE")
        }
        os.environ.pop("KINQHI_YOLO_MODE", None)
        os.environ.pop("KINQHI_INTERACTIVE", None)
        # KINQHI_CRON_SESSION takes priority over KINQHI_GATEWAY_SESSION in
        ...
        os.environ.pop("KINQHI_CRON_SESSION", None)
        os.environ["KINQHI_GATEWAY_SESSION"] = "1"
        os.environ["KINQHI_SESSION_KEY"] = self.SESSION_KEY
