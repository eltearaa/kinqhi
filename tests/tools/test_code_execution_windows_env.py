...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``KINQHI_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # KINQHI_* handling (#27303): only the operational allowlist passes;
        # every other KINQHI_* is dropped (the broad prefix was removed).
        "KINQHI_HOME": "/home/alice/.hermes",        # allowlisted → kept
        "KINQHI_PROFILE": "default",                 # allowlisted → kept
        "KINQHI_INTERACTIVE": "1",                   # not allowlisted → dropped
        "KINQHI_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "KINQHI_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
