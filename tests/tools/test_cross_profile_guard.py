...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile Kinqhi layout and point KINQHI_HOME at
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up KINQHI_HOME (set in
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.kinqhi/profiles/<name>/."""
        # Don't set KINQHI_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_kinqhi_home_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        ...
        assert "Active Kinqhi profile" in src
        assert "cross_profile=True" in src
        assert "~/.kinqhi/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Kinqhi profile: default" in src
        assert "Active Kinqhi profile: {active_profile}" in src
