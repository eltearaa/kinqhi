...
def _clean_state():
    ...
    saved = {}
    for k in ("KINQHI_INTERACTIVE", "KINQHI_GATEWAY_SESSION", "KINQHI_EXEC_ASK", "KINQHI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["KINQHI_INTERACTIVE"] = "1"
        session_key = os.getenv("KINQHI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No KINQHI_INTERACTIVE or KINQHI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        os.environ["KINQHI_SESSION_KEY"] = session_key
        try:
            check_all_command_guards(command, "local")
        finally:
            os.environ.pop("KINQHI_GATEWAY_SESSION", None)
            os.environ.pop("KINQHI_EXEC_ASK", None)
            os.environ.pop("KINQHI_SESSION_KEY", None)
