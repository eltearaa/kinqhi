...
def test_check_website_access_uses_dynamic_kinqhi_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new KINQHI_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old KINQHI_HOME set by the autouse _isolate_kinqhi_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing KINQHI_HOME to tmp_path
