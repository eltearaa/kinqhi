...
into the shared $KINQHI_HOME, and must heal a stale 'docker' stamp left there
by older images.

Background (shared-$KINQHI_HOME bug)
------------------------------------
$KINQHI_HOME (/opt/data) is a DATA volume that users commonly bind-mount from
the host (``~/.kinqhi:/opt/data``) and sometimes share with a host-side
Desktop/CLI install. Older images wrote ``printf 'docker' > $KINQHI_HOME/.install_method``
at boot, which clobbered the host install's own marker — so the host's in-app
updater read 'docker' and refused to run ``kinqhi update`` ("doesn't apply
...
``detect_install_method``). stage2 must therefore:
  ...
  * NOT write the 'docker' stamp into $KINQHI_HOME any more, and
  * proactively remove a stale 'docker' stamp from $KINQHI_HOME so homes
...
def test_stage2_does_not_write_install_method_into_home(stage2_text: str) -> None:
    ...
    assert not re.search(
        r"(tee|>)\s*\"?\$KINQHI_HOME/\.install_method", stage2_text
    ), (
        "stage2 must not stamp $KINQHI_HOME/.install_method — that data dir "
...
def test_stage2_heals_stale_docker_home_stamp(stage2_text: str) -> None:
    # It must remove a stale 'docker' stamp from $KINQHI_HOME so already
    # poisoned shared homes recover.
    assert 'rm -f "$KINQHI_HOME/.install_method"' in stage2_text, (
        "stage2 must remove a stale 'docker' stamp from $KINQHI_HOME to heal "
