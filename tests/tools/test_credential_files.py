...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        kinqhi_home.mkdir()
        (kinqhi_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(kinqhi_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        kinqhi_home = tmp_path / ".hermes"
        (kinqhi_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape KINQHI_HOME."""
        monkeypatch.setenv("KINQHI_HOME", str(tmp_path / ".hermes"))
        (tmp_path / ".hermes").mkdir()

        # Create a sensitive file one level above kinqhi_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside kinqhi_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside KINQHI_HOME must still be registered."""
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
        (kinqhi_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_kinqhi_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of KINQHI_HOME must be allowed."""
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        subdir = kinqhi_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside KINQHI_HOME pointing outside must be rejected."""
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))

        # Create a sensitive file outside kinqhi_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside kinqhi_home pointing outside
        symlink = kinqhi_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes KINQHI_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, kinqhi_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape KINQHI_HOME."""
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
        ...
        sensitive.write_text("{}")
        self._write_config(kinqhi_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(kinqhi_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside KINQHI_HOME via config must still mount."""
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))

        (kinqhi_home / "oauth.json").write_text("{}")
        self._write_config(kinqhi_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        (kinqhi_home / "cache" / "documents").mkdir(parents=True)
        (kinqhi_home / "cache" / "audio").mkdir(parents=True)
        (kinqhi_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        kinqhi_home.mkdir()
        # Use legacy dir name — get_kinqhi_dir prefers old if it exists
        (kinqhi_home / "document_cache").mkdir()
        (kinqhi_home / "image_cache").mkdir()
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(kinqhi_home / "document_cache") in host_paths
        assert str(kinqhi_home / "image_cache") in host_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        kinqhi_home = tmp_path / ".hermes"
        img_dir = kinqhi_home / "cache" / "images"
    ...
    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        kinqhi_home = tmp_path / ".hermes"
        (kinqhi_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("KINQHI_HOME", str(kinqhi_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(kinqhi_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        doc_dir = kinqhi_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        kinqhi_home = tmp_path / ".hermes"
        ss_dir = kinqhi_home / "cache" / "screenshots"
