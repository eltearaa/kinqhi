...
def _restore_tool_modules():
    original_kinqhi_home = os.environ.get("KINQHI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_kinqhi_home is None:
            os.environ.pop("KINQHI_HOME", None)
        else:
            os.environ["KINQHI_HOME"] = original_kinqhi_home
        _reset_modules(("tools", "kinqhi_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kinqhi_cli = types.ModuleType("kinqhi_cli")
    kinqhi_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kinqhi_cli"] = kinqhi_cli
    kinqhi_home = tmp_path / "hermes-home"
    os.environ["KINQHI_HOME"] = str(kinqhi_home)
    sys.modules["kinqhi_cli.config"] = types.SimpleNamespace(
        get_kinqhi_home=lambda: kinqhi_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kinqhi", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": kinqhi_home / "modal_snapshots.json",
