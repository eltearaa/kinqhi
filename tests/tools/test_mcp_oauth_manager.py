...
def test_hermes_provider_subclass_exists():
    """KinqhiMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _KINQHI_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _KINQHI_PROVIDER_CLS is not None
    assert issubclass(_KINQHI_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hermes_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns KinqhiMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _KINQHI_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("KINQHI_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _KINQHI_PROVIDER_CLS is not None
    assert isinstance(provider, _KINQHI_PROVIDER_CLS)
