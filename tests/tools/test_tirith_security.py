...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(kinqhi_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(kinqhi_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in KINQHI_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
# ---------------------------------------------------------------------------
# KINQHI_HOME isolation
# ---------------------------------------------------------------------------

class TestKinqhiHomeIsolation:
    def test_hermes_bin_dir_respects_kinqhi_home(self):
        """_hermes_bin_dir must use KINQHI_HOME, not hardcoded ~/.kinqhi."""
        from tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"KINQHI_HOME": tmpdir}):
            result = _hermes_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_kinqhi_home(self):
        """_failure_marker_path must use KINQHI_HOME, not hardcoded ~/.kinqhi."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"KINQHI_HOME": "/custom/hermes"}):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets KINQHI_HOME; verify it's active."""
        kinqhi_home = os.getenv("KINQHI_HOME")
        assert kinqhi_home is not None, "KINQHI_HOME should be set by conftest"
        assert "hermes_test" in kinqhi_home, "Should point to test temp dir"

    def test_get_kinqhi_home_fallback(self):
        """Without KINQHI_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_kinqhi_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove KINQHI_HOME entirely. With HOME also absent, expanduser
