...
KINQHI_GATEWAY_BOOTSTRAP_STATE on first boot, so a freshly-provisioned
...
KINQHI_GATEWAY_BOOTSTRAP_STATE=running; stage2-hook.sh (installed as
...
This mirrors the existing KINQHI_AUTH_JSON_BOOTSTRAP env-seed pattern: it seeds
...
def test_seed_block_present_and_guarded(stage2_text: str) -> None:
    ...
    assert '[ ! -f "$KINQHI_HOME/gateway_state.json" ]' in block, (
        "seed must be guarded by [ ! -f ] so persisted state wins on restart"
    )
    assert "KINQHI_GATEWAY_BOOTSTRAP_STATE" in block
...
def _run_seed(
    ...
    """Run the extracted seed block in a sandbox $KINQHI_HOME.

    ``env_value`` is the KINQHI_GATEWAY_BOOTSTRAP_STATE value (None = unset).
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        env_line = (
            f'export KINQHI_GATEWAY_BOOTSTRAP_STATE="{env_value}"\n'
            if env_value is not None
            else "unset KINQHI_GATEWAY_BOOTSTRAP_STATE\n"
        )
        script = (
            "set -e\n"
            f'KINQHI_HOME="{home}"\n'
...
def test_no_seed_when_env_unset(stage2_text: str) -> None:
    ...
    assert out is None, "seed must not run when KINQHI_GATEWAY_BOOTSTRAP_STATE is unset"
