...
The canonical transcript store is SQLite via `SessionDB` (from `kinqhi_state`). The
...
Counts consecutive restarts via a JSON file (`{KINQHI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `kinqhi update`, `kinqhi gateway restart`,
or `/restart`.
