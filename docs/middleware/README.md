...
Kinqhi middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Kinqhi'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Kinqhi calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Kinqhi retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Kinqhi guardrails,
...
- `telemetry_schema_version`: currently `kinqhi.observer.v1`
- `middleware_schema_version`: currently `kinqhi.middleware.v1`
...
Kinqhi stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Kinqhi runs
them as a nested chain in registration order. Middleware failures are fail-open:
Kinqhi logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Kinqhi applies middleware in this order:
...
For each tool call, Kinqhi applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Kinqhi pre-execution path against the effective arguments:
...
```bash
kinqhi plugins enable <plugin-name>
```

For isolated local testing, use one `KINQHI_HOME` for plugin enablement and the
agent run:

```bash
export KINQHI_HOME=/tmp/kinqhi-middleware-test
mkdir -p "$KINQHI_HOME"
kinqhi plugins enable <plugin-name>
kinqhi chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run kinqhi plugins enable <plugin-name>
uv run kinqhi chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/kinqhi-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Kinqhi expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Kinqhi treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Kinqhi preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Kinqhi does not convert downstream
