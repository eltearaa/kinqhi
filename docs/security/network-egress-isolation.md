...
When running Kinqhi inside Docker, the default `network_mode: host` gives the
...
The Kinqhi [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   KINQHI_UID=$(id -u) KINQHI_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=kinqhi,kinqhi-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://kinqhi-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Kinqhi trust model and vulnerability reporting
