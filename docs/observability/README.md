# Kinqhi Observer Hooks

Kinqhi observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Kinqhi' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "kinqhi.observer.v1"
```

Hook callbacks are fail-open. Kinqhi catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Kinqhi only builds
