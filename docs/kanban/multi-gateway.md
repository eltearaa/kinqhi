...
Kinqhi supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.kinqhi/config.yaml`:
...
Or set the env var: `KINQHI_KANBAN_DISPATCH_IN_GATEWAY=false`
