"""
Kinqhi MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Kinqhi-specific extra)

Usage:
    kinqhi mcp serve
    kinqhi mcp serve --verbose
...
logger = logging.getLogger("kinqhi.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using KINQHI_HOME."""
    try:
        from kinqhi_constants import get_kinqhi_home
        return get_kinqhi_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("KINQHI_HOME", Path.home() / ".kinqhi")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from kinqhi_constants import get_kinqhi_home
        directory_file = get_kinqhi_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("KINQHI_HOME", Path.home() / ".kinqhi")
...
class EventBridge:
    ...
    This is the Kinqhi equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from kinqhi_constants import get_kinqhi_home
            db_file = get_kinqhi_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("KINQHI_HOME", Path.home() / ".kinqhi")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Kinqhi MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "kinqhi",
        instructions=(
            "Kinqhi messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Kinqhi MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
