# Kinqhi Security Policy

This document describes Kinqhi's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/kinqhi/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Kinqhi does not operate a bug
bounty program.**
...
- Environment details (`kinqhi version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Kinqhi access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Kinqhi's documentation
...
Kinqhi supports two OS-level isolation postures. They address
...
Kinqhi supports this in two ways:

- **Kinqhi's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Kinqhi's in-process heuristics
...
Kinqhi filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Kinqhi
itself. Bugs in Kinqhi's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Kinqhi does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Kinqhi's own documentation, or reasonable
  operator expectations would predict — including cases where
  Kinqhi has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  kinqhi-home security, and similar. Reports against those
...
- Kinqhi includes supply-chain guards for MCP server
