...
`mcp_tool.py`, and `kinqhi_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# KinqhiMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------
...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class KinqhiMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`KinqhiTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, KinqhiTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import KinqhiTokenStorage
                        if isinstance(storage, KinqhiTokenStorage):
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import KinqhiTokenStorage
            if not isinstance(storage, KinqhiTokenStorage):
                return
    ...
    return KinqhiMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_KINQHI_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`KinqhiMCPOAuthProvider` directly using the helpers
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `kinqhi mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``kinqhi mcp remove <name>`` and (indirectly) by
        ``kinqhi mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
