#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Kinqhi Skills Hub.
...
Used by kinqhi_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("kinqhi", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.kinqhi/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("kinqhi", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Kinqhi centralized index source
# ---------------------------------------------------------------------------

KINQHI_INDEX_URL = "https://kinqhi.nousresearch.com/docs/api/skills-index.json"
KINQHI_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "kinqhi-index.json"
KINQHI_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for KINQHI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if KINQHI_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - KINQHI_INDEX_CACHE_FILE.stat().st_mtime
            if age < KINQHI_INDEX_TTL:
                return json.loads(KINQHI_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(KINQHI_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Kinqhi index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Kinqhi index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        KINQHI_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        KINQHI_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class KinqhiIndexSource(SkillSource):
    """Skill source backed by the centralized Kinqhi Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "kinqhi-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "kinqhi-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        KinqhiIndexSource(auth=auth), # Centralized index (search + resolved install paths)
