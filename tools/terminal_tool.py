...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Kinqhi exit
...
# display_kinqhi_home imported lazily at call site (stale-module safety during kinqhi update)
...
def _get_scratch_dir(*a, **kw): return None  # singularity removed
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of kinqhi directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "kinqhi-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("KINQHI_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("KINQHI_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("KINQHI_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (KINQHI_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (KINQHI_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Kinqhi password prompt or the sudo -S password-pipe path.
...
class _SingularityEnvironment: pass  # singularity removed
from tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from tools.environments.docker import DockerEnvironment as _DockerEnvironment
class _ModalEnvironment: pass  # modal removed
class _DaytonaEnvironment: pass  # daytona removed
class _ManagedModalEnvironment: pass  # managed_modal removed
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Kinqhi can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``kinqhi=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Kinqhi processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Kinqhi processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Kinqhi processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, KinqhiSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.kinqhi/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for kinqhi-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Kinqhi processes that hit SIGKILL / OOM / a closed terminal
        ...
        _DaytonaEnvironment = type('_DaytonaEnvironment', (), {})  # daytona removed
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Kinqhi can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/kinqhi
        # restart|stop targeting kinqhi-gateway) must never run inside the
        ...
        # never restart. This mirrors the `kinqhi gateway restart` guard in
        # kinqhi_cli/gateway.py and the cron-path guard in kinqhi_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_KINQHI_GATEWAY") == "1":
            from kinqhi_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `kinqhi gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # kinqhi dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in kinqhi dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("KINQHI_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("KINQHI_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("KINQHI_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("KINQHI_SESSION_USER_ID", "")
                        _gw_user_name = _gse("KINQHI_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("KINQHI_SESSION_MESSAGE_ID", "")
