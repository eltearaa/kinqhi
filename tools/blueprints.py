...
`kinqhi skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.kinqhi.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.kinqhi.blueprint`` is a mapping containing
    ...
    kinqhi = meta.get("kinqhi") if isinstance(meta, dict) else None
    blueprint = kinqhi.get("blueprint") if isinstance(kinqhi, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.kinqhi.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.kinqhi.blueprint`` block) they can hand
    to ``kinqhi skills publish`` to share. ``body`` is the plain-language
