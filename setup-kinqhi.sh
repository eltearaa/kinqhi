...
# 5. Symlinks the 'kinqhi' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/kinqhi-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink kinqhi into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up kinqhi command..."

KINQHI_BIN="$SCRIPT_DIR/venv/bin/kinqhi"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KINQHI_BIN" "$COMMAND_LINK_DIR/kinqhi"
echo -e "${GREEN}✓${NC} Symlinked kinqhi → $COMMAND_LINK_DISPLAY_DIR/kinqhi"
...
# ============================================================================
# Seed bundled skills into ~/.kinqhi/skills/
# ============================================================================

KINQHI_SKILLS_DIR="${KINQHI_HOME:-$HOME/.kinqhi}/skills"
mkdir -p "$KINQHI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.kinqhi/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KINQHI_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  kinqhi status        # Check configuration"
if is_termux; then
    echo "  kinqhi gateway       # Run gateway in foreground"
else
    echo "  kinqhi gateway install # Install gateway service (messaging + cron)"
fi
echo "  kinqhi cron list     # View scheduled jobs"
echo "  kinqhi doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m kinqhi_cli.main setup
fi
