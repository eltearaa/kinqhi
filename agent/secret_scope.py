...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the KINQHI_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Kinqhi runtime / deployment
    "KINQHI_HOME", "KINQHI_PROFILE", "KINQHI_GATEWAY_LOCK_DIR",
    "KINQHI_MAX_ITERATIONS", "KINQHI_MAX_TOKENS", "KINQHI_API_TIMEOUT",
    "KINQHI_REDACT_SECRETS", "KINQHI_NOUS_TIMEOUT_SECONDS",
    "_KINQHI_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "KINQHI_KANBAN_DB", "KINQHI_KANBAN_WORKSPACES_ROOT", "KINQHI_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "KINQHI_KANBAN_",
    "KINQHI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(kinqhi_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(kinqhi_home) / ".env")
