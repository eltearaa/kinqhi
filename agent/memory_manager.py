...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_KINQHI_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Kinqhi expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``kinqhi_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_kinqhi_home()`` themselves.
        """
        if "kinqhi_home" not in kwargs:
            from kinqhi_constants import get_kinqhi_home
            kwargs["kinqhi_home"] = str(get_kinqhi_home())
        for provider in self._providers:
            try:
