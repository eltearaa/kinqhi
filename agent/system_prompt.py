...
upstream prefix cache warm.  See ``kinqhi-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Kinqhi never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want KINQHI_HOME persona while keeping
    ...
    # Pointer to the kinqhi skill + docs for user questions about Kinqhi itself.
    stable_parts.append(KINQHI_AGENT_HELP_GUIDANCE)
    ...
    # KINQHI_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Kinqhi, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Kinqhi profile the agent is running
    # under so it doesn't conflate ~/.kinqhi/skills/ (default profile) with
    # ~/.kinqhi/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Kinqhi profile: default. Other profiles (if any) live "
            "under ~/.kinqhi/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Kinqhi profile: {active_profile}. This session reads "
            f"and writes ~/.kinqhi/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.kinqhi/skills/, ~/.kinqhi/plugins/, "
            f"~/.kinqhi/cron/, ~/.kinqhi/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Kinqhi never rebuilds or reinjects parts of it
