...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Kinqhi starts and finds ``~/.openclaw/``.

    Points users at ``kinqhi claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kinqhi claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Kinqhi, run "
        "`kinqhi claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `kinqhi claw cleanup` (renames it to ~/.openclaw.pre-migration — "
