...
``auth.json``.  They intentionally have no dependency on ``kinqhi_cli.auth`` so
...
# Sources Kinqhi owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Kinqhi-owned OAuth/device-code state)
