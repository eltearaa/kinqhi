...
Plugins built on Kinqhi often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Kinqhi
...
:class:`~kinqhi_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Kinqhi' main config
...
handles every provider, fallback chain, and per-task override Kinqhi
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`kinqhi_cli.plugins.PluginContext`
