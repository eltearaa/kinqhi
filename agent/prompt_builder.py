...
from kinqhi_constants import get_kinqhi_home, get_skills_dir, is_wsl
...
_KINQHI_MD_NAMES = (".kinqhi.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.kinqhi.md`` or ``HERMES.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Kinqhi, an intelligent AI assistant created by Nous Research. "
...
KINQHI_AGENT_HELP_GUIDANCE = (
    "You run on Kinqhi (by Nous Research). When the user needs help with "
    "Kinqhi itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://kinqhi.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `kinqhi` skill with skill_view(name='kinqhi') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.kinqhi/kanban.db`. Your task id is in "
    "`$KINQHI_KANBAN_TASK`; your workspace is `$KINQHI_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $KINQHI_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `kinqhi kanban <verb>` for board operations. Use "
...
#
# The kinqhi runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to kinqhi's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Kinqhi "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Kinqhi WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Kinqhi itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Kinqhi runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Kinqhi itself is running. The host OS, home, and cwd "
                f"of the Kinqhi process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Kinqhi "
    ...
    # Kinqhi desktop GUI — any agent running under the desktop app should know
    # it. KINQHI_DESKTOP marks the backend powering the chat; KINQHI_DESKTOP_TERMINAL
    # marks a kinqhi launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("KINQHI_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("KINQHI_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Kinqhi desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Kinqhi
    ...
    extra = (os.getenv("KINQHI_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_kinqhi_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.kinqhi/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("KINQHI_PLATFORM")
        or get_session_env("KINQHI_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Kinqhi itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `kinqhi` skill "
            "first. It has the actual commands (e.g. `kinqhi config set …`, `kinqhi tools`, "
            "`kinqhi setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: kinqhi setup, kinqhi setup tools, kinqhi setup terminal, kinqhi status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from KINQHI_HOME and return its content, or None.
    ...
    """
    try:
        from kinqhi_cli.config import ensure_kinqhi_home
        ensure_kinqhi_home()
    except Exception as e:
        logger.debug("Could not ensure KINQHI_HOME before loading SOUL.md: %s", e)

    soul_path = get_kinqhi_home() / "SOUL.md"
...
def _load_hermes_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.kinqhi.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        return _truncate_content(
            result, ".kinqhi.md", context_length=context_length,
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .kinqhi.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from KINQHI_HOME is independent and always included when present.
    ...
    # SOUL.md from KINQHI_HOME only — skip when already loaded as identity
    if not skip_soul:
