...
as ``kind: backend``) or ``~/.kinqhi/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``kinqhi tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$KINQHI_HOME/cache/videos/``, creating parents as needed."""
    from kinqhi_constants import get_kinqhi_home

    path = get_kinqhi_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$KINQHI_HOME/cache/videos/``.
