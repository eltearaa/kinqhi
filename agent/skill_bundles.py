...
Storage
-------
Bundles live in ``~/.kinqhi/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``KINQHI_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``kinqhi bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KINQHI_HOME.

    Honors ``KINQHI_BUNDLES_DIR`` for tests; falls back to
    ``<KINQHI_HOME>/skill-bundles``.
    """
    override = os.environ.get("KINQHI_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_kinqhi_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `kinqhi bundles` CLI subcommand.
# ---------------------------------------------------------------------------
