...
def _kinqhi_home_path() -> Path:
    """Resolve the active KINQHI_HOME (profile-aware) without circular imports."""
    try:
        from kinqhi_constants import get_kinqhi_home  # local import to avoid cycles
        return get_kinqhi_home()
    except Exception:
        return Path(os.path.expanduser("~/.kinqhi"))


def _hermes_root_path() -> Path:
    """Resolve the Kinqhi root dir (always the parent of any profile, never per-profile)."""
    try:
        from kinqhi_constants import get_default_hermes_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(kinqhi_home / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved KINQHI_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("KINQHI_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    hermes_dirs = []
    for base in (_kinqhi_home_path(), _hermes_root_path()):
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Kinqhi path.

    Three categories are blocked:

      * Internal Kinqhi cache files under ``KINQHI_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under KINQHI_HOME and the global Kinqhi
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.kinqhi/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active KINQHI_HOME (profile-aware) AND the global
    # Kinqhi root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (KINQHI_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Kinqhi cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Kinqhi credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Kinqhi MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Kinqhi MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Kinqhi profiles are separate KINQHI_HOME dirs under
...
#
# Reference: May 2026 incident where a kinqhi-security profile session
# edited skills under both ``~/.kinqhi/profiles/kinqhi-security/skills/``
# AND ``~/.kinqhi/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under KINQHI_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from KINQHI_HOME.
    ...
    """
    try:
        home_real = _kinqhi_home_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Kinqhi scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Kinqhi entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Kinqhi profile {info['target_profile']!r}, but the "
...
#
#   <KINQHI_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.kinqhi/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.kinqhi/…`` segment and warns
# regardless of which Kinqhi profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.kinqhi/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.kinqhi`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.kinqhi/…`` and returns the
    index where the inner Kinqhi-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .kinqhi / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".kinqhi":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Kinqhi state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.kinqhi``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.kinqhi`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Kinqhi resolver to
    succeed, so it works correctly even when called from contexts where
    KINQHI_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Kinqhi process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real KINQHI_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.kinqhi/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.kinqhi/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host KINQHI_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Kinqhi state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Kinqhi "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real KINQHI_HOME. Use the "
