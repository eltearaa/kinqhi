...
#
# When a user invokes a /skill (or /bundle), Kinqhi expands the turn into a
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``KINQHI_PLATFORM`` env var and
    ``KINQHI_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("KINQHI_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        ...
        if identifier_path.is_absolute():
            ...
            # find a skill via ~/.kinqhi/skills/<name> where <name> is a
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.kinqhi.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_kinqhi_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.kinqhi/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.kinqhi/skills/`` and any ``skills.external_dirs`` so the
