...
Owns one Codex thread per Kinqhi session. Drives `turn/start`, consumes
...
# Kinqhi' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_KINQHI_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Kinqhi/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Kinqhi session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _KINQHI_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("KINQHI_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Kinqhi' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Kinqhi' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own kinqhi-tools callback we
            # auto-accept — the user already approved Kinqhi' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Kinqhi approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Kinqhi returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort Kinqhi version string for codex's userAgent line."""
    try:
