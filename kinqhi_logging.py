"""Centralized logging setup for Kinqhi.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.kinqhi/logs/`` (profile-aware via ``get_kinqhi_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``kinqhi_cli.web_server``,
    ``kinqhi_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Kinqhi (TUI, gateway, ``hy_memory`` server, MCP
...
from kinqhi_constants import get_config_path, get_kinqhi_home
...
# Used by _ComponentFilter and exposed for ``kinqhi logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    kinqhi_home: Optional[Path] = None,
    ...
    """Configure the Kinqhi logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    kinqhi_home
        Override for the Kinqhi home directory.  Falls back to
        ``get_kinqhi_home()`` (profile-aware).
    log_level
    ...
    home = kinqhi_home or get_kinqhi_home()
    log_dir = home / "logs"
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the kinqhi group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from kinqhi_cli.config import is_managed
