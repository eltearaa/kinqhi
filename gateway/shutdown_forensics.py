...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if kinqhi_home_str:
            takeover_path = Path(kinqhi_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(kinqhi_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded kinqhi but never re-ran ``kinqhi setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../kinqhi-gateway.service"
    ...
    # that's the common case for kinqhi.
    ...
    # constant in kinqhi_cli/gateway.py.
    headroom = 30.0
