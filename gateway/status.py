...
The PID file lives at ``{KINQHI_HOME}/gateway.pid``.  KINQHI_HOME defaults to
``~/.kinqhi`` but can be overridden via the environment variable.  This means
separate KINQHI_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting KINQHI_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("KINQHI_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "kinqhi" / _LOCKS_DIRNAME
...
def looks_like_gateway_command_line(command: str | None) -> bool:
    ...
    test also matched ``kinqhi_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\kinqhi-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Kinqhi's
    ...
    for token in tokens:
        ...
        if basename in ("kinqhi-gateway", "kinqhi-gateway.exe"):
            return True

    joined = " ".join(tokens)
    has_gateway_entry = (
        "kinqhi_cli.main" in joined
        or "kinqhi_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("kinqhi", "kinqhi.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return True  # bare `kinqhi gateway` defaults to `run`
        return filtered[i + 1] == "run"
    return False


def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Kinqhi gateway."""
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `kinqhi status` show per-profile
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different KINQHI_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``kinqhi.service`` + ``kinqhi-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``kinqhi gateway stop`` on Windows would be misclassified as an
