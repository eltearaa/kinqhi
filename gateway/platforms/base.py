...
# Audio file extensions Kinqhi recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Kinqhi' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Kinqhi-created Telegram private-chat
...
from kinqhi_constants import get_default_hermes_root, get_kinqhi_dir, get_kinqhi_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.kinqhi/.env manually."
)
...
# Default location: {KINQHI_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_kinqhi_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; KinqhiAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_kinqhi_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_kinqhi_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_kinqhi_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_kinqhi_dir("cache/screenshots", "browser_screenshots")
_KINQHI_HOME = get_kinqhi_home()
_KINQHI_ROOT = get_default_hermes_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "KINQHI_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "KINQHI_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "KINQHI_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "KINQHI_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _KINQHI_HOME / "image_cache",
    _KINQHI_HOME / "audio_cache",
    _KINQHI_HOME / "video_cache",
    _KINQHI_HOME / "document_cache",
    _KINQHI_HOME / "browser_screenshots",
    ...
    _KINQHI_HOME / "cache" / "images",
    _KINQHI_HOME / "cache" / "audio",
    _KINQHI_HOME / "cache" / "videos",
    _KINQHI_HOME / "cache" / "documents",
    _KINQHI_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Kinqhi profile and shared Kinqhi root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for hermes_root in (_KINQHI_HOME, _KINQHI_ROOT):
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Kinqhi
    secrets (``~/.kinqhi/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``KINQHI_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Kinqhi-managed cache, under an operator-allowlisted root
    (``KINQHI_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.kinqhi/.env,
    # ~/.kinqhi/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?kinqhi\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+kinqhi[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Kinqhi commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("KINQHI_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "KINQHI_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "KINQHI_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Kinqhi sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.kinqhi/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from kinqhi_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          KINQHI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          KINQHI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          KINQHI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("KINQHI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("KINQHI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("KINQHI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
