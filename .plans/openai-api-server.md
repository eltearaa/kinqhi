# OpenAI-Compatible API Server for Kinqhi
...
SSE streaming. By exposing this endpoint, kinqhi becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kinqhi/.env`
2. Run `kinqhi gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kinqhi through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kinqhi as a model) |
...
Authorization: Bearer kinqhi-api-key-here
...
But kinqhi sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kinqhi"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kinqhi model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kinqhi's core prompt
...
Once implemented, kinqhi works as a drop-in backend for:
