...
React + Ink terminal UI for Kinqhi. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `KINQHI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.kinqhi/.hermes_history` or under `KINQHI_HOME`.
...
```text
ui-tui/
  packages/kinqhi-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      kinqhi-ink.d.ts    type declarations for @kinqhi/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent KinqhiCLI subprocess for slash commands
```
