...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.KINQHI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Kinqhi on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via KINQHI_TUI_TRUECOLOR=1.
