#!/usr/bin/env python3
"""Google Workspace API CLI for Kinqhi.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Kinqhi-facing JSON contract and falls back to the Python client
...
from _kinqhi_home import get_kinqhi_home
...
def _gws_binary() -> str | None:
    override = os.getenv("KINQHI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Kinqhi")
