...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hermes_env="${KINQHI_HOME:-$HOME/.kinqhi}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env"; then
...
```bash
source "${KINQHI_HOME:-$HOME/.kinqhi}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Kinqhi. Code looks clean — good test coverage, no security concerns."
