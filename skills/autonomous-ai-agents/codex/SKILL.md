...
metadata:
  kinqhi:
    ...
    related_skills: [claude-code, kinqhi]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Kinqhi terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Kinqhi itself, `model.provider: openai-codex` uses Kinqhi-managed Codex
OAuth from `~/.kinqhi/auth.json` after `kinqhi auth add openai-codex`. For the
...
## Kinqhi Gateway Caveat

When invoking the Codex CLI from a Kinqhi gateway/service context (for example,
