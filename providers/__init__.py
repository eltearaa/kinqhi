...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with kinqhi)
2. User plugins: ``$KINQHI_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$KINQHI_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$KINQHI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from kinqhi_constants import get_kinqhi_home

        d = get_kinqhi_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple KINQHI_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$KINQHI_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with kinqhi.
    ...
    # 2. User plugins — under $KINQHI_HOME/plugins/model-providers/<name>/.
