"""``kinqhi debug`` debug tools for Kinqhi.

Currently supports:
    kinqhi debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.kinqhi/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[kinqhi debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.kinqhi/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``kinqhi debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``kinqhi debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Kinqhi version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `kinqhi debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "kinqhi/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``kinqhi debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.kinqhi/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``kinqhi debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----KinqhiDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from kinqhi_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_kinqhi_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``kinqhi dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``kinqhi dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``kinqhi debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "kinqhi debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `kinqhi debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  kinqhi debug delete <url>")

    print(f"\nShare these links with the Kinqhi team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: kinqhi debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'kinqhi debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``kinqhi debug`` call.
    ...
    # best-effort — any failure is swallowed so ``kinqhi debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: kinqhi debug <command>")
        print()
        print("Commands:")
