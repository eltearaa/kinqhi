...
- Keep data: Remove code but keep ~/.kinqhi/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Kinqhi PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing kinqhi or kinqhi PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Kinqhi" comment and following line
                if '# Kinqhi' in line or '# kinqhi' in line:
                    skip_next = True
                    continue
                if skip_next and ('kinqhi' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing kinqhi
                if 'kinqhi' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the kinqhi wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "kinqhi",
        Path("/usr/local/bin/kinqhi"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains kinqhi_cli reference)
                content = wrapper.read_text()
                if 'kinqhi_cli' in content or 'kinqhi' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(kinqhi_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``kinqhi`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Kinqhi home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (kinqhi_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``kinqhi gateway run`` processes
    ...
    try:
        from kinqhi_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from kinqhi_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``KINQHI_HOME`` and ``KINQHI_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\kinqhi\git\cmd``, ``%LOCALAPPDATA%\kinqhi\git\bin``,
#      ``%LOCALAPPDATA%\kinqhi\git\usr\bin``, ``%LOCALAPPDATA%\kinqhi\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\kinqhi\git\`` and Node to
#      ``%LOCALAPPDATA%\kinqhi\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``kinqhi dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\kinqhi\gateway-service\`` and sometimes
...
def _hermes_path_markers(kinqhi_home: Path) -> list[str]:
    """Path-entry substrings that identify Kinqhi-owned User-PATH entries."""
    root = str(kinqhi_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare kinqhi install dir.
    markers = [root + "\\kinqhi", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if KINQHI_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "kinqhi".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-kinqhi as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(kinqhi_home: Path) -> list[str]:
    """Strip Kinqhi-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(kinqhi_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete KINQHI_HOME and KINQHI_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("KINQHI_HOME", "KINQHI_GIT_BASH_PATH"):
                try:
...
def _is_default_kinqhi_home(kinqhi_home: Path) -> bool:
    """Return True when ``kinqhi_home`` points at the default (non-profile) root."""
    try:
        from kinqhi_constants import get_default_hermes_root
        return kinqhi_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its KINQHI_HOME directory.

    We shell out to ``kinqhi -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current KINQHI_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m kinqhi_cli.main` so we don't depend on a `kinqhi`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "kinqhi_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's KINQHI_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``kinqhi uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$KINQHI_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from kinqhi_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(kinqhi_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Kinqhi Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Kinqhi Chat GUI installation was found.")
        print(f"  Checked: {kinqhi_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Kinqhi agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(kinqhi_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Kinqhi agent at {kinqhi_home / 'kinqhi'}")
        print(f"  • Your config, sessions, and secrets under {kinqhi_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Kinqhi agent is still installed. Run 'kinqhi' to use the CLI,")
    print("or 'kinqhi uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.kinqhi/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.kinqhi/ for future reinstall
    """
    project_root = get_project_root()
    kinqhi_home = get_kinqhi_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie KINQHI_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_kinqhi_home(kinqhi_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.kinqhi data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Kinqhi Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {kinqhi_home / 'config.yaml'}")
    print(f"  Secrets: {kinqhi_home / '.env'}")
    print(f"  Data:    {kinqhi_home / 'cron/'}, {kinqhi_home / 'sessions/'}, {kinqhi_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their KINQHI_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Kinqhi data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Kinqhi code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``kinqhi`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$KINQHI_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in kinqhi_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\kinqhi\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(kinqhi_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Kinqhi-owned PATH entries in User environment")

        log_info("Removing KINQHI_HOME / KINQHI_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Kinqhi-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing kinqhi command...")
    ...
    #     (only when they still point into this Kinqhi home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Kinqhi-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(kinqhi_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Kinqhi-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(kinqhi_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE KINQHI_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from kinqhi_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(kinqhi_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.kinqhi/, just remove the kinqhi subdir
            if kinqhi_home in project_root.parents or project_root.parent == kinqhi_home:
    ...
    #     under KINQHI_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(kinqhi_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(kinqhi_home)
    ...
    # 5. Optionally remove ~/.kinqhi/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile KINQHI_HOME dirs live under
        ...
        try:
            if kinqhi_home.exists():
                shutil.rmtree(kinqhi_home)
                log_success(f"Removed {kinqhi_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {kinqhi_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {kinqhi_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://kinqhi.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://kinqhi.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Kinqhi! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m kinqhi_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import kinqhi_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``kinqhi_constants`` + ``kinqhi_cli.colors``
    (and lazily ``kinqhi_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m kinqhi_cli.uninstall")
    parser.add_argument(
        "--mode",
