"""Profile distributions — shareable, packaged Kinqhi profiles via git.

A distribution is a Kinqhi profile published as a git repository (or
...
* ``kinqhi profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``kinqhi skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``kinqhi profile``, not a parallel tree):

    kinqhi profile install <source> [--name N] [--alias] [--force] [--yes]
    kinqhi profile update  <name>  [--force-config] [--yes]
    kinqhi profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "kinqhi_state.db", "response_store.db",
    ...
    "backups", "cache",
    # Infrastructure
    "kinqhi", ".worktrees", "profiles", "bin", "node_modules",
...
def check_hermes_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Kinqhi {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Kinqhi distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``kinqhi profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Kinqhi profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Kinqhi distribution."
        )
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.kinqhi).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
    ...
    with tempfile.TemporaryDirectory(prefix="hermes_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `kinqhi profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`kinqhi profile install <source> --name {canon} --force`."
        )
