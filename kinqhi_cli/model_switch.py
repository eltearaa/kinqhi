...
- ``kinqhi_cli.providers``        -- canonical provider identity + overlays
- ``kinqhi_cli.model_normalize``  -- per-provider name formatting
...
_KINQHI_MODEL_WARNING = (
    "Nous Research Kinqhi 3 & 4 models are NOT agentic and are not designed "
    "for use with Kinqhi. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Kinqhi 3 / Kinqhi 4 chat families.
# The previous substring check (`"kinqhi" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``kinqhi-brain:qwen3-14b-ctx16k`` that just
# happen to carry "kinqhi" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Kinqhi-3-Llama-3.1-70B, kinqhi-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   kinqhi-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_KINQHI_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])kinqhi[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Kinqhi 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_KINQHI_NON_AGENTIC_RE.search(model_name))


def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Kinqhi 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _KINQHI_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``kinqhi config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from kinqhi_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kinqhi model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kinqhi doctor' — config issues detected:"
        ...
        from kinqhi_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from kinqhi_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from kinqhi_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in kinqhi_cli.runtime_provider.resolve_runtime_provider;
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from kinqhi_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from kinqhi_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from kinqhi_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by kinqhi provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://kinqhi.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Kinqhi release. Falls back to the in-repo
    ...
    # --- 1. Check Kinqhi-mapped providers ---
    from kinqhi_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from kinqhi_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (KINQHI_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from kinqhi_cli.auth import _load_auth_store
        ...
        # /model picker sees the SAME list `kinqhi model` would build, with
    ...
    # --- 2. Check Kinqhi-only providers (nous, openai-codex, copilot, opencode-go) ---
    from kinqhi_cli.providers import KINQHI_OVERLAYS
    from kinqhi_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Kinqhi provider ID.
    # KINQHI_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Kinqhi IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in KINQHI_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Kinqhi slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `kinqhi model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from kinqhi_cli.auth import get_provider_auth_state as _nous_state
    ...
    # in PROVIDER_TO_MODELS_DEV or KINQHI_OVERLAYS (keeps /model in sync
    # with `kinqhi model`).
    try:
        from kinqhi_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Kinqhi's canonical write key (matches
            ...
            # Kinqhi writes ``models:`` as a dict keyed by model id
            # (see kinqhi_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Kinqhi's own writer uses; a hyphen variant is accepted
            ...
            # active model. Kinqhi's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, kinqhi_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`kinqhi_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from kinqhi_cli.models import fetch_openrouter_models
