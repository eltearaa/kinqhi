"""
kinqhi fallback — manage the fallback provider chain.
...
Subcommands:
  kinqhi fallback [list]   Show the current fallback chain (default when no subcommand)
  kinqhi fallback add      Pick provider + model via the same picker as `kinqhi model`,
                           then append the selection to the chain
  kinqhi fallback remove   Pick an entry to delete from the chain
  kinqhi fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.kinqhi/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from kinqhi_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `kinqhi model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  kinqhi fallback add")
        print()
        return
    ...
    print("  Docs: https://kinqhi.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `kinqhi model`, then append the selection to the chain."""
    from kinqhi_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `kinqhi model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `kinqhi fallback list` to view, or `kinqhi fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``kinqhi fallback [subcommand]``."""
