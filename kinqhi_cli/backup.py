"""
Backup and import commands for kinqhi CLI.

`kinqhi backup` creates a zip archive of the entire ~/.kinqhi/ directory
(excluding the kinqhi repo and transient files).

`kinqhi import` restores from a backup zip, overlaying onto the current
KINQHI_HOME root.
"""
...
from kinqhi_constants import get_default_hermes_root, get_kinqhi_home, display_kinqhi_home
...
# ``kinqhi`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/kinqhi/``
...
# KINQHI_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "kinqhi",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under KINQHI_HOME) —
...
# File names that ``kinqhi import`` must never overwrite, matched by basename so
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to kinqhi root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``kinqhi`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/kinqhi/`` — must be preserved.
        if part == "kinqhi" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside KINQHI_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Kinqhi home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Kinqhi home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"kinqhi-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"kinqhi-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_kinqhi_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``kinqhi`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "kinqhi" and not is_root)
        ]
    ...
    print(f"\nRestore with: kinqhi import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Kinqhi backup.
    ...
    # Look for telltale files that a kinqhi home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Kinqhi backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.kinqhi/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a kinqhi dir name
        if prefix in {".kinqhi", "kinqhi"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_kinqhi_home()}")
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Kinqhi configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # kinqhi_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: kinqhi profile list  (after installing kinqhi)")

        # Guidance
        print()
        if not (hermes_root / "kinqhi").is_dir():
            print("Note: The kinqhi codebase was not included in the backup.")
            print("  If this is a fresh install, run: kinqhi update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  kinqhi -p {pname} gateway install")

        print("Done. Your Kinqhi configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and kinqhi backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to KINQHI_HOME).
...
# — `kinqhi update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(kinqhi_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(kinqhi_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside KINQHI_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``kinqhi update``.
    ...
    Args:
        ...
        kinqhi_home: Override for the Kinqhi home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(kinqhi_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for kinqhi backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_kinqhi_home()}/state-snapshots/")
...
def _pre_update_backup_dir(kinqhi_home: Optional[Path] = None) -> Path:
    home = kinqhi_home or get_kinqhi_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of KINQHI_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<KINQHI_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``kinqhi update``) should continue even if the backup fails.
    """
    hermes_root = kinqhi_home or get_default_hermes_root()
    if not hermes_root.is_dir():
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `kinqhi claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of KINQHI_HOME under ``backups/`` before a
    ``kinqhi claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``kinqhi import <archive>``.  Writes to
    ``<KINQHI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `kinqhi import` and the
