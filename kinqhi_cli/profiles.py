"""
Profile management for multiple isolated Kinqhi instances.

Each profile is a fully independent KINQHI_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.kinqhi/profiles/<name>/`` by default.

The "default" profile is ``~/.kinqhi`` itself — backward compatible,
zero migration needed.

Usage::

    kinqhi profile create coder          # fresh profile + bundled skills
    kinqhi profile create coder --clone  # also copy config, .env, SOUL.md, skills
    kinqhi profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    kinqhi -p coder chat                 # or via flag
    kinqhi profile use coder             # set as sticky default
    kinqhi profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See kinqhi_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `kinqhi backup` archives
...
# Marker file written by `kinqhi profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `kinqhi
...
# `kinqhi skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Kinqhi infrastructure directories that only the default profile
         (``~/.kinqhi``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_kinqhi_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.kinqhi) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kinqhi",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "kinqhi", "default", "test", "tmp", "root", "sudo",
})

# Kinqhi subcommands that cannot be used as profile names/aliases
_KINQHI_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the kinqhi root, NOT to the current KINQHI_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KINQHI_HOME points outside
    ``~/.kinqhi``, profiles live under ``KINQHI_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_kinqhi_home() / "profiles"


def _get_default_kinqhi_home() -> Path:
    """Return the default (pre-profile) KINQHI_HOME path.

    In standard deployments this is ``~/.kinqhi``.
    In Docker/custom deployments where KINQHI_HOME is outside ``~/.kinqhi``
    (e.g. ``/opt/data``), returns KINQHI_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_kinqhi_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Kinqhi installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KINQHI_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_kinqhi_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, kinqhi subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _KINQHI_SUBCOMMANDS:
        return f"'{canon}' conflicts with a kinqhi subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            hermes_exe = shutil.which("kinqhi") or "kinqhi"
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Kinqhi. If we
    ...
    try:
        ...
        # not be migrated. The next `kinqhi doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``kinqhi -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``kinqhi profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``kinqhi profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Kinqhi config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kinqhi_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, kinqhi_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own KINQHI_HOME.
    ...
    The returned ``kinqhi_home`` is the path to pass to
    ``set_kinqhi_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_kinqhi_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``kinqhi update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kinqhi)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.kinqhi/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `kinqhi -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Kinqhi profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `kinqhi update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`kinqhi profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Kinqhi (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `kinqhi profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `kinqhi -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KINQHI_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``kinqhi profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KINQHI_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_kinqhi_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kinqhi).\n"
            "To remove everything, use: kinqhi uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``KINQHI_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from kinqhi_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KINQHI_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KINQHI_HOME")
    try:
        os.environ["KINQHI_HOME"] = str(profile_dir)
        from kinqhi_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KINQHI_HOME"] = old_home
        elif "KINQHI_HOME" in os.environ:
            del os.environ["KINQHI_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kinqhi/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: kinqhi profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KINQHI_HOME.

    Returns ``"default"`` if KINQHI_HOME is not set or points to ``~/.kinqhi``.
    Returns the profile name if KINQHI_HOME points into ``~/.kinqhi/profiles/<name>``.
    Returns ``"custom"`` if KINQHI_HOME is set to an unrecognized path.
    """
    from kinqhi_constants import get_kinqhi_home
    kinqhi_home = get_kinqhi_home()
    resolved = kinqhi_home.resolve()

    default_resolved = _get_default_kinqhi_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.kinqhi itself — its parent is ~/ and its
        # directory name is ".kinqhi", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kinqhi profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.kinqhi itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kinqhi). "
            "Specify a different name: kinqhi profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"kinqhi.{old_name}"
    new_host = f"hermes_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_kinqhi_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KINQHI_HOME path string.

    Called early in the CLI entry point, before any kinqhi modules
    are imported, to set the KINQHI_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
