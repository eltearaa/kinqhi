"""Windows ConPTY bridge for the `kinqhi dashboard` chat tab.

Drop-in counterpart to ``kinqhi_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``kinqhi_cli.web_server`` consumes: ``spawn``, ``read``,
