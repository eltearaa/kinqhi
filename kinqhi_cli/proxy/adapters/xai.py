...
# xAI's public API is OpenAI-compatible for the endpoints Kinqhi commonly
# uses. The Responses endpoint is included because Kinqhi' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Kinqhi-managed OAuth credentials."""

    auth_hint = "kinqhi auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`kinqhi auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`kinqhi auth reset xai-oauth` or re-authenticate with "
                    "`kinqhi auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `kinqhi auth add xai-oauth --type oauth`."
            )
