"""``kinqhi debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Kinqhi. Use 'kinqhi debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    kinqhi debug share              Upload debug report and print URL
    kinqhi debug share --lines 500  Include more log lines
    kinqhi debug share --expire 30  Keep paste for 30 days
    kinqhi debug share --local      Print report locally (no upload)
    kinqhi debug share --no-redact  Disable upload-time secret redaction
    kinqhi debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'kinqhi debug share'",
    )
