"""``kinqhi dashboard`` subcommand parser.

Extracted verbatim from ``kinqhi_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Kinqhi web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `kinqhi dashboard` cmdlines and SIGTERM them directly — the same
    # path `kinqhi update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running kinqhi dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running kinqhi dashboard processes and exit",
    )
    # Backward-compat shim: older Kinqhi desktop app shells (<= 0.15.x) spawn the
    # backend as `kinqhi dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Kinqhi couldn't start" with no actionable cause. Accept and silently
    ...
    # `kinqhi dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.kinqhi/.env.
    # Nested subparser so bare `kinqhi dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "KINQHI_DASHBOARD_OAUTH_CLIENT_ID into ~/.kinqhi/.env, and prints "
            "how to engage the login gate. Requires being logged in (kinqhi setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://kinqhi.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via KINQHI_DASHBOARD_PORTAL_URL. Mainly for "
