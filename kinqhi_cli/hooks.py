"""kinqhi hooks — inspect and manage shell-script hooks.

Usage::

    kinqhi hooks list
    kinqhi hooks test <event> [--for-tool X] [--payload-file F]
    kinqhi hooks revoke <command>
    kinqhi hooks doctor

Consent records live under ``~/.kinqhi/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.kinqhi/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``kinqhi hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: kinqhi hooks {list|test|revoke|doctor}")
        print("Run 'kinqhi hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.kinqhi/config.yaml.")
        print("See `kinqhi hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `kinqhi hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `kinqhi hooks test` and `kinqhi hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Kinqhi wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `kinqhi hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `kinqhi hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `kinqhi hooks doctor`.")
