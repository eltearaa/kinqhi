"""
Baked-in build metadata for Kinqhi.

Source installs report their git revision live via ``git rev-parse`` (see
``kinqhi_cli/dump.py`` and ``kinqhi_cli/banner.py``).  That doesn't work inside
...
To make ``kinqhi dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$KINQHI_GIT_SHA``
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``KINQHI_GIT_SHA`` build-arg fall through to live-git
...
def get_build_sha(short: int = 8) -> Optional[str]:
    ...
    written by the Dockerfile's ``KINQHI_GIT_SHA`` build-arg and contains
