"""Slash command definitions and autocomplete for the Kinqhi CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Kinqhi config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Kinqhi works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Kinqhi is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.kinqhi/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Kinqhi to the latest version", "Info"),
    CommandDef("version", "Show Kinqhi version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`kinqhi_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/kinqhi`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Kinqhi
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``kinqhi skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kinqhi skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``kinqhi skills list`` and
...
# unrelated command landed. These claim their slots right after /kinqhi,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /kinqhi <command>).
...
# native slot, the alias spelling stays reachable via /kinqhi reset).
...
# ``/kinqhi <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/kinqhi" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /kinqhi credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /kinqhi billing.
#   - debug: the log/report upload surface; reached via /kinqhi debug on Slack.
_SLACK_VIA_KINQHI_ONLY = frozenset({"credits", "billing", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/kinqhi <verb>`` subcommand.
    ...
    can still reach them via ``/kinqhi <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/kinqhi`` is always reserved as the first entry so the
    legacy ``/kinqhi <subcommand>`` form keeps working for anything that
    ...
    # Reserve /kinqhi as the catch-all top-level command.
    entries.append(("kinqhi", "Talk to Kinqhi or run a subcommand", "[subcommand] [args]"))
    seen.add("kinqhi")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_KINQHI_ONLY:
            # Intentionally Slack-via-/kinqhi only (see _SLACK_VIA_KINQHI_ONLY).
            return
    ...
    # everything except /kinqhi, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://kinqhi.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /kinqhi handler.

    Maps both canonical names and aliases so /kinqhi bg do stuff works
    the same as /kinqhi background do stuff.

    Plugin-registered slash commands are included so ``/kinqhi <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from kinqhi_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
