"""CLI subcommand: ``kinqhi send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"kinqhi send: {file_path} is not a text file. --file reads the "
                ...
                f'  kinqhi send --to telegram "MEDIA:{file_path}"\n'
                f'  kinqhi send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  kinqhi send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"kinqhi send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"kinqhi send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"kinqhi send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"kinqhi send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"kinqhi send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `kinqhi gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.kinqhi/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.kinqhi/.env`` AND bridge top-level
    ...
    startup that ``kinqhi send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.kinqhi/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.kinqhi/config.yaml`` into
       ...
       via ``kinqhi config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``kinqhi send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.kinqhi/.env and ~/.kinqhi/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "kinqhi send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  kinqhi send --to telegram \"hello\"\n"
            "  kinqhi send --to discord:#ops --file report.md\n"
            "  kinqhi send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "kinqhi send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `kinqhi send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Kinqhi "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.kinqhi/.env + ~/.kinqhi/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  kinqhi send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | kinqhi send --to telegram:-1001234567890\n"
            "  kinqhi send --to discord:#ops --file /tmp/report.md\n"
            "  kinqhi send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  kinqhi send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  kinqhi send --list                  # all platforms\n"
            "  kinqhi send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `kinqhi send --list telegram`.",
    )
