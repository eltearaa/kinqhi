"""``kinqhi portal`` — the human-readable entry point for Nous Portal.

Running ``kinqhi portal`` with no subcommand performs the one-shot Portal
...
``kinqhi auth add nous --type oauth`` (which still works), is identical to
``kinqhi setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``kinqhi auth`` or ``kinqhi tools``. It's the onboarding + discovery
...
DOCS_URL = "https://kinqhi.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `kinqhi model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `kinqhi portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `kinqhi auth add nous --type
    oauth`. It reuses the exact wiring behind `kinqhi setup --portal` (which in
    ...
    """
    from kinqhi_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `kinqhi portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `kinqhi portal` is the
        # human-readable alias for `kinqhi auth add nous --type oauth` /
        # `kinqhi setup --portal`.
    ...
    print("Run `kinqhi portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `kinqhi portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `kinqhi portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `kinqhi auth add "
            "nous --type oauth`, identical to `kinqhi setup --portal`). "
