"""
Kinqhi Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``kinqhi desktop`` / ``kinqhi gui``)
     Built inside the agent checkout under ``$KINQHI_HOME/kinqhi/``:
       ...
       - ``$KINQHI_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Kinqhi.app`` or ``~/Applications/Kinqhi.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Kinqhi`` (NSIS per-user)
...
the app name ("Kinqhi"), separate from ``$KINQHI_HOME``:
  - macOS:   ``~/Library/Application Support/Kinqhi``
  - Windows: ``%APPDATA%\\Kinqhi``
  - Linux:   ``$XDG_CONFIG_HOME/Kinqhi`` (default ``~/.config/Kinqhi``)
...
``kinqhi uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(kinqhi_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return kinqhi_home / "kinqhi"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Kinqhi"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Kinqhi"
...
def source_built_gui_artifacts(kinqhi_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``kinqhi desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``kinqhi/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(kinqhi_home)
    ...
    return [
        ...
        kinqhi_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Kinqhi" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Kinqhi.app"),
            home / "Applications" / "Kinqhi.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Kinqhi).
            local_base / "Programs" / "Kinqhi",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Kinqhi")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Kinqhi.desktop",
        ]
    return paths


def agent_is_installed(kinqhi_home: Path) -> bool:
    """Return True when a usable Python agent install exists under KINQHI_HOME.
    ...
    if (agent_root / "kinqhi_cli").is_dir():
        return True
...
def gui_is_installed(kinqhi_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(kinqhi_home):
        if p.exists():
            return True
...
def gui_install_summary(kinqhi_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = kinqhi_home if kinqhi_home is not None else get_kinqhi_home()
...
def uninstall_gui(kinqhi_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``kinqhi/kinqhi_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$KINQHI_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove kinqhi' or "
            "'sudo dnf remove kinqhi'). AppImage builds are a single file you "
