"""
MCP Server Management CLI — ``kinqhi mcp`` subcommand.

Implements ``kinqhi mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.kinqhi/config.yaml under the ``mcp_servers`` key.
"""
...
from kinqhi_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_kinqhi_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from kinqhi_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.kinqhi/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``kinqhi mcp login`` to distinguish a genuine authentication
    ...
    try:
        ...
        return KinqhiTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # kinqhi_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  kinqhi mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  kinqhi mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  kinqhi mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kinqhi_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: kinqhi mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_kinqhi_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `kinqhi mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kinqhi mcp add <name> --url <endpoint>')
        _info('  kinqhi mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `kinqhi mcp remove` + `kinqhi mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `kinqhi mcp login " + name + "`.")
            return
        if tools:
...
# ─── kinqhi mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'kinqhi mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``kinqhi mcp`` subcommands."""
    ...
    if action == "picker":
        from kinqhi_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from kinqhi_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from kinqhi_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `kinqhi plugin`.
        from kinqhi_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("kinqhi mcp                                    Open the catalog picker (default)")
        _info("kinqhi mcp catalog                            List Nous-approved MCPs")
        _info("kinqhi mcp install <name>                     Install a catalog MCP")
        _info("kinqhi mcp serve                              Run as MCP server")
        _info("kinqhi mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("kinqhi mcp add <name> --command <cmd>         Add a stdio server")
        _info("kinqhi mcp add <name> --preset <preset>       Add from a known preset")
        _info("kinqhi mcp remove <name>                      Remove a server")
        _info("kinqhi mcp list                               List configured servers")
        _info("kinqhi mcp test <name>                        Test connection")
        _info("kinqhi mcp configure <name>                   Toggle tools")
        _info("kinqhi mcp login <name>                       Re-authenticate OAuth")
        print()
