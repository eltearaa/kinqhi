...
manual BotFather token copy-paste. Kinqhi talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.kinqhi.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "KinqhiSetupBot"

DEFAULT_BOT_NAME = "Kinqhi"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Kinqhi Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Kinqhi Telegram onboarding service.")
