"""kinqhi webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    kinqhi webhook subscribe <name> [options]
    kinqhi webhook list
    kinqhi webhook remove <name>
    kinqhi webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.kinqhi/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: kinqhi gateway run
"""
...
def webhook_command(args):
    """Entry point for 'kinqhi webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: kinqhi webhook {subscribe|list|remove|test}")
        print("Run 'kinqhi webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (kinqhi gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: kinqhi webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from kinqhi webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (kinqhi gateway run)")
