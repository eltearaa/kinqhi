"""MCP picker — interactive `kinqhi mcp picker` (also the default `kinqhi mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``kinqhi mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `kinqhi plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from kinqhi_cli.cli_output import prompt_yes_no
from kinqhi_cli.curses_ui import curses_single_select
from kinqhi_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Kinqhi session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from kinqhi_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `kinqhi mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: kinqhi mcp install <name>    Picker: kinqhi mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Kinqhi is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Kinqhi — run `kinqhi update` "
...
def show_catalog() -> None:
    """`kinqhi mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`kinqhi mcp picker` (and default `kinqhi mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`kinqhi mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `kinqhi mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
