"""
Dump command for kinqhi CLI.

Outputs a compact, plain-text summary of the user's Kinqhi setup
...
from kinqhi_cli.config import get_kinqhi_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``KINQHI_GIT_SHA`` build-arg (see ``kinqhi_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``kinqhi dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from kinqhi_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(kinqhi_home: Path) -> int:
...
def _cron_summary(kinqhi_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = kinqhi_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"kinqhi_home:      {display_kinqhi_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `kinqhi auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `kinqhi auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["kinqhi-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(kinqhi_home)}")
    lines.append(f"  skills:             {_count_skills(kinqhi_home)}")
