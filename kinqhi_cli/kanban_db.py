...
``<root>`` is the **shared Kinqhi root** (the parent of any active
...
``kinqhi -p <profile>`` joins the same board as the dispatcher that
...
the profile's Kinqhi home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``KINQHI_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``KINQHI_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``kinqhi kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.kinqhi``. In Docker / custom
deployments where ``KINQHI_HOME`` points outside ``~/.kinqhi`` (e.g.
``/opt/kinqhi``), ``<root>`` is ``KINQHI_HOME``. Legacy env-var
overrides still work:

* ``KINQHI_KANBAN_DB`` — pin the database file path directly.
* ``KINQHI_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``KINQHI_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``KINQHI_KANBAN_DB``,
``KINQHI_KANBAN_WORKSPACES_ROOT``, and ``KINQHI_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/kinqhi-kanban-v1-spec.pdf`` for the full
...
# ``KINQHI_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``KINQHI_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("KINQHI_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``KINQHI_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("KINQHI_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``KINQHI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "KINQHI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
# enough that kebab-case names like ``atm10-server`` or ``kinqhi``
...
def kanban_home() -> Path:
    """Return the shared Kinqhi root that anchors the kanban board.

    Resolution order:

    1. ``KINQHI_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``KINQHI_HOME`` is ``<root>/profiles/<name>``, and returns
       ``KINQHI_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``KINQHI_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("KINQHI_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``kinqhi kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``KINQHI_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``kinqhi kanban boards
    ...
    env = os.environ.get("KINQHI_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``kinqhi kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``KINQHI_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("KINQHI_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``KINQHI_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("KINQHI_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``KINQHI_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("KINQHI_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``kinqhi kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``KINQHI_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``KINQHI_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("KINQHI_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Kinqhi treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``KINQHI_KANBAN_DB`` env → ``KINQHI_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See kinqhi_state._WAL_INCOMPAT_MARKERS for detection logic.
                from kinqhi_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``kinqhi kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``KINQHI_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from kinqhi_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``kinqhi --skills ...`` alongside the built-in
    ...
    # `kinqhi --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``kinqhi kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `kinqhi kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``kinqhi kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``kinqhi kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``kinqhi kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``kinqhi kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``KINQHI_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Kinqhi' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Kinqhi-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``kinqhi kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Kinqhi checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Kinqhi treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Kinqhi creates/reuses that linked worktree. With
      no ``workspace_path``, Kinqhi anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Kinqhi uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``KINQHI_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Kinqhi
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``kinqhi kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``kinqhi kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Kinqhi profile.
    ...
    if not rows:
        return False
    try:
        from kinqhi_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from kinqhi_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Kinqhi profile.
        # `_default_spawn` invokes ``kinqhi -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Kinqhi
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `kinqhi kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Kinqhi CLI invocation."""
    # ``kinqhi_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``kinqhi`` package — there is no
    # ``kinqhi`` package to import.
    return [sys.executable, "-m", "kinqhi_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Kinqhi shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Kinqhi executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``kinqhi`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$KINQHI_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("kinqhi")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\kinqhi.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m kinqhi_cli.main`` — fallback for setups where
       Kinqhi is launched from a venv and the ``kinqhi`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``kinqhi_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("KINQHI_BIN", "").strip()
    if env_bin:
    ...
    hermes_bin = _safe_which_no_cwd("kinqhi") if _IS_WINDOWS else shutil.which("kinqhi")
    if hermes_bin:
        return _hermes_path_argv(hermes_bin)
    return _module_hermes_argv()


def _kanban_worker_skill_available(kinqhi_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``kinqhi -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset KINQHI_HOME means the worker falls back to the default root
    # home (``~/.kinqhi``), which ships the bundled skill.
    base = _Path(kinqhi_home) if kinqhi_home else (_Path.home() / ".kinqhi")
...
def _resolve_worker_cli_toolsets(kinqhi_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``KINQHI_KANBAN_TASK`` is set.
    """
    if not kinqhi_home:
        return None
    try:
        from kinqhi_constants import reset_kinqhi_home_override, set_kinqhi_home_override
        from kinqhi_cli.config import load_config
        from kinqhi_cli.tools_config import _get_platform_tools

        token = set_kinqhi_home_override(kinqhi_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for KINQHI_HOME=%r (%s)",
            kinqhi_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``kinqhi -p <profile> chat -q ...`` subprocess.
    ...
    ``KINQHI_KANBAN_DB`` / ``KINQHI_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject KINQHI_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `kinqhi -p <name>` the
    # _apply_profile_override() runs *before* kinqhi_constants is imported.
    # If KINQHI_HOME is absent from the child's env, get_kinqhi_home() falls
    # back to Path.home() / ".kinqhi" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from kinqhi_cli.profiles import resolve_profile_env
    try:
        env["KINQHI_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via KINQHI_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # KINQHI_HOME never had profiles created.
        pass
    if task.tenant:
        env["KINQHI_TENANT"] = task.tenant
    env["KINQHI_KANBAN_TASK"] = task.id
    env["KINQHI_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["KINQHI_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["KINQHI_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["KINQHI_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["KINQHI_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["KINQHI_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`kinqhi -p <name>`
    # rewrites KINQHI_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["KINQHI_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["KINQHI_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["KINQHI_KANBAN_BOARD"] = resolved_board
    # KINQHI_PROFILE is the author the kanban_comment tool defaults to.
    # `kinqhi -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["KINQHI_PROFILE"] = profile_arg

    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped KINQHI_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("KINQHI_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("KINQHI_HOME"))
    if worker_toolsets:
    ...
    # `kinqhi kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`kinqhi` executable not found on PATH. "
            "Install Kinqhi or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``kinqhi kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled KINQHI_PROFILE values like "kinqhi-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Kinqhi root exists

    Reads profile paths directly so this module has no import dependency on
    ``kinqhi_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from kinqhi_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``kinqhi kanban assignees`` for the terminal.
