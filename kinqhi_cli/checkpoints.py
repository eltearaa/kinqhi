"""`kinqhi checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.kinqhi/checkpoints/``.  Actions:

    kinqhi checkpoints               # same as `status`
    kinqhi checkpoints status        # total size, project count, breakdown
    kinqhi checkpoints list          # per-project checkpoint counts + workdir
    kinqhi checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    kinqhi checkpoints clear [-f]    # nuke the entire base (asks first)
    kinqhi checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    kinqhi checkpoints
    kinqhi checkpoints prune --retention-days 3 --max-size-mb 200
    kinqhi checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: kinqhi checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``kinqhi checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `kinqhi checkpoints` → status
