"""Migrate Kinqhi' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Kinqhi' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Kinqhi mcp_servers.<n>.command/args/env  → codex stdio transport
  Kinqhi mcp_servers.<n>.url/headers       → codex streamable_http transport
  Kinqhi mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Kinqhi mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Kinqhi-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Kinqhi' own
...
MIGRATION_MARKER = (
    "# managed by kinqhi — `kinqhi codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end kinqhi managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Kinqhi config.")
...
# Kinqhi keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Kinqhi' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Kinqhi MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Kinqhi' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_KINQHI_KEYS:
            skipped.append(f"{key} (unknown Kinqhi key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (KINQHI_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Kinqhi)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Kinqhi' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Kinqhi' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Kinqhi
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="kinqhi-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a KINQHI_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed kinqhi-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.kinqhi`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Kinqhi' own
    ...
    so a kinqhi installed under /opt/, /usr/local/, or a venv all work.
    KINQHI_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # KINQHI_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_kinqhi_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever KINQHI_HOME its launcher sets
    ...
    # — that would override the launcher's KINQHI_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("KINQHI_HOME", tmp_path) would
    ...
    kinqhi_home = os.environ.get("KINQHI_HOME") or ""
    if kinqhi_home and _looks_like_test_tempdir(kinqhi_home):
        kinqhi_home = ""
    if kinqhi_home:
        env["KINQHI_HOME"] = kinqhi_home
    # PYTHONPATH passes through so a worktree-launched kinqhi finds the
    ...
    env["KINQHI_QUIET"] = "1"
    env["KINQHI_REDACT_SECRETS"] = env.get("KINQHI_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Kinqhi mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.kinqhi/config.yaml dict
        ...
        expose_hermes_tools: when True (default), register Kinqhi' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Kinqhi for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Kinqhi config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Kinqhi' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Kinqhi for the tools codex
    ...
    if expose_hermes_tools:
        translated["kinqhi-tools"] = _build_hermes_tools_mcp_entry()
        if "kinqhi-tools" not in report.migrated:
            report.migrated.append("kinqhi-tools")
