...
on Japanese locales, etc.).  Kinqhi's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``kinqhi_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Kinqhi is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KINQHI_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KINQHI_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Kinqhi-managed Git install directories so
    ...
    # already-running shells don't see the change — which means kinqhi
    ...
    # install without Kinqhi-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Kinqhi's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Kinqhi's
    ``kinqhi config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Kinqhi (User env var in Windows Settings, or
    export in a PowerShell profile) and Kinqhi picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Kinqhi-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\kinqhi\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``kinqhi`` from right after install) retain their old PATH.

    Any subprocess Kinqhi spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Kinqhi-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "kinqhi", "git", "cmd"),
        os.path.join(local_appdata, "kinqhi", "git", "bin"),
        os.path.join(local_appdata, "kinqhi", "git", "usr", "bin"),
        # Kinqhi venv Scripts directory — host of the kinqhi.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes kinqhi, but harmless to include.
        os.path.join(local_appdata, "kinqhi", "kinqhi", "venv", "Scripts"),
