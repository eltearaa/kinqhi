...
Pure logic: walks a Kinqhi config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `kinqhi doctor` and a future `kinqhi migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Kinqhi config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Kinqhi config and return retirement issues.
