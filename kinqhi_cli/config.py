"""
Configuration management for Kinqhi.

Config files are stored in ~/.kinqhi/ for easy access:
- ~/.kinqhi/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.kinqhi/.env         - API keys and secrets

This module provides:
- kinqhi config          - Show current configuration
- kinqhi config edit     - Open config in editor
- kinqhi config set      - Set a specific value
- kinqhi config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``kinqhi config set`` (which rewrites
    ...
    ``config.yaml`` in place: kinqhi never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.kinqhi/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``kinqhi logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  kinqhi config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Kinqhi makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Kinqhi itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``kinqhi update``, the TUI build.
...
#   on every plugin install / ``kinqhi update``.
...
# * ``KINQHI_HOME`` / ``KINQHI_PROFILE`` / ``KINQHI_CONFIG`` /
#   ``KINQHI_ENV`` — Kinqhi runtime location flags. Writing these into
...
#
# IMPORTANT: ``KINQHI_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (KINQHI_GEMINI_CLIENT_ID,
# KINQHI_LANGFUSE_PUBLIC_KEY, KINQHI_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Kinqhi runtime location — never via dashboard env writer.
    # NOT a KINQHI_* blanket: integration credentials (KINQHI_GEMINI_*,
    # KINQHI_LANGFUSE_*, KINQHI_SPOTIFY_*, ...) ARE allowed.
    "KINQHI_HOME", "KINQHI_PROFILE", "KINQHI_CONFIG", "KINQHI_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Kinqhi runtime location "
            "(KINQHI_HOME, KINQHI_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "KINQHI_TOOL_PROGRESS", "KINQHI_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `kinqhi plugins enable
    # observability/langfuse` or `kinqhi tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "KINQHI_LANGFUSE_ENV",
    "KINQHI_LANGFUSE_RELEASE",
    "KINQHI_LANGFUSE_SAMPLE_RATE",
    "KINQHI_LANGFUSE_MAX_CHARS",
    "KINQHI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from kinqhi_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("KINQHI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_kinqhi_home() / ".managed"
    if managed_marker.exists():
        return "NixOS"
    return None

def is_managed() -> bool:
    """Check if Kinqhi is running in package-manager-managed mode.

    Two signals: the KINQHI_MANAGED env var (set by the systemd service),
    or a .managed marker file in KINQHI_HOME (set by the NixOS activation
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``kinqhi_cli/`` — i.e. the git checkout for source
    installs, ``/opt/kinqhi`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$KINQHI_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Kinqhi was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$KINQHI_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. KINQHI_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.kinqhi``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$KINQHI_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.kinqhi:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$KINQHI_HOME``, a
    ...
    ``docker`` and ``kinqhi update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/kinqhi`` image bakes a ``docker``
        stamp into ``/opt/kinqhi`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $KINQHI_HOME.
    try:
    ...
    #    host install whose shared $KINQHI_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``kinqhi update``.
    try:
        method = (
            (get_kinqhi_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``kinqhi_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$KINQHI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/kinqhi`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Kinqhi lives in a ``uv tool`` layout.

    ``uv tool install kinqhi`` places the install at
    ``.../uv/tools/kinqhi/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``kinqhi`` happens to be uv-tool-installed on the machine while
    the *active* Kinqhi is a regular pip/venv install, causing
    ``kinqhi update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/kinqhi/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade kinqhi"
    if method == "docker":
        return "docker pull nousresearch/kinqhi:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade kinqhi"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade kinqhi"
        return "pip install --upgrade kinqhi"
    return "kinqhi update"
...
# Long-form text for ``kinqhi update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# kinqhi_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Kinqhi, it doesn't update
...
✗ ``kinqhi update`` doesn't apply inside the Docker container.

Kinqhi runs as a published image (nousresearch/kinqhi), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/kinqhi:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate kinqhi
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/kinqhi:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/kinqhi/tags
  • Your config and session history live under ``$KINQHI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``kinqhi update`` inside Docker.
...
def format_managed_message(action: str = "modify this Kinqhi installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("KINQHI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Kinqhi installation is managed by NixOS "
            f"(KINQHI_MANAGED={env_hint}).\n"
            "Edit services.kinqhi.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Kinqhi installation is managed by Homebrew "
            f"(KINQHI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade kinqhi"
        )
    ...
    return (
        f"Cannot {action}: this Kinqhi installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Kinqhi."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KINQHI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or KINQHI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("KINQHI_DEV") == "1":
        return None
    ...
    container_mode_file = get_kinqhi_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "kinqhi")
    exec_user = info.get("exec_user", "kinqhi")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/kinqhi")
...
# Re-export from kinqhi_constants — canonical definition lives there.
from kinqhi_constants import get_kinqhi_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kinqhi_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_kinqhi_home() / ".env"
...
def _resolve_hermes_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the KINQHI_UID / KINQHI_GID env vars set by Docker deployments.

    Docker containers running Kinqhi commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level KINQHI_HOME once, but
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("KINQHI_UID", "").strip()
    gid_str = os.environ.get("KINQHI_GID", "").strip()
    try:
...
def _chown_to_hermes_uid(path) -> None:
    """Chown ``path`` to ``KINQHI_UID:KINQHI_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_kinqhi_home` on Docker deployments.
    See #34107.
    """
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kinqhi group can
    share state with the gateway service.

    The mode can be overridden via the KINQHI_HOME_MODE environment variable
    (e.g. KINQHI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KINQHI_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``KINQHI_UID``/``KINQHI_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("KINQHI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Kinqhi runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("KINQHI_CONTAINER") or os.environ.get("KINQHI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set KINQHI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KINQHI_HOME if the user doesn't have one yet."""
...
def ensure_kinqhi_home():
    """Ensure ~/.kinqhi directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_kinqhi_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_kinqhi_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"KINQHI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Kinqhi-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Kinqhi
        ...
        # identity slot (SOUL.md). Empty by default. The KINQHI_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Kinqhi adds a coding operating brief
    ...
    "terminal": {
        ...
        #             KINQHI_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force KINQHI_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Kinqhi auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Kinqhi sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Kinqhi
        ...
        #  "/home/user/.kinqhi/cache/documents:/output"]
        ...
        # bundled Kinqhi image, which drops to the `kinqhi` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Kinqhi sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Kinqhi should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``kinqhi chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.kinqhi/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: kinqhi sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .kinqhi.md, or .cursorrules before Kinqhi applies
    ...
    # single read_file page exceeds these limits, Kinqhi truncates the
    ...
    # See tools/kanban_tools.py and kinqhi_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``kinqhi dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``kinqhi kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``kinqhi kanban decompose`` and the kanban
        ...
        # ``kinqhi profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `kinqhi model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `kinqhi` (and `kinqhi chat`) launches by default:
        ...
        # REPL and `--tui` (or KINQHI_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `kinqhi --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kinqhi -c` muscle memory.  Default off so existing
        # users aren't surprised.  KINQHI_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `kinqhi --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/kinqhi`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``KINQHI_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``KINQHI_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``KINQHI_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``KINQHI_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/kinqhi``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Kinqhi appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kinqhi/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kinqhi/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Kinqhi feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Kinqhi auto-pauses the goal and
    ...
    # always goes to ~/.kinqhi/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${KINQHI_SKILL_DIR} and ${KINQHI_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `kinqhi curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `kinqhi curator run --consolidate` overrides
        ...
        # normally restored on every `kinqhi update`, so pruning them only
        ...
        # skipped), snapshot ~/.kinqhi/skills/ into
        # ~/.kinqhi/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `kinqhi curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for kinqhi-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Kinqhi*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KINQHI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Kinqhi' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.kinqhi/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or KINQHI_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `kinqhi doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``kinqhi_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Kinqhi to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $KINQHI_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via KINQHI_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `kinqhi -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `kinqhi` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `kinqhi kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     kinqhi's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Kinqhi tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.kinqhi/logs/.
    ...
    # update model picker lists without shipping a kinqhi release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://kinqhi.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `kinqhi model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.kinqhi/.env,
        ...
        # behavior: files must live under the Kinqhi cache, under
        ...
        # user. Bridged to KINQHI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Kinqhi
        # cache (~/.kinqhi/cache/{documents,images,audio,video,screenshots})
        ...
        # to KINQHI_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # KINQHI_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to KINQHI_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.kinqhi/state.db.
    ...
    "sessions": {
        ...
        # default of ``kinqhi sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.kinqhi/sessions/session_{sid}.json`` on every turn
    ...
    # ``kinqhi update`` behaviour.
    "updates": {
        # Run a full ``kinqhi backup``-style zip of KINQHI_HOME before every
        # ``kinqhi update``.  Backups land in ``<KINQHI_HOME>/backups/`` and
        # can be restored with ``kinqhi import <path>``.  Defaults to true
        # after the #48200 incident: a ``kinqhi update --yes`` run that
        ...
        "backup_keep": 5,
        # What `kinqhi update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<KINQHI_HOME>/lsp/bin/`` on first use.
    ...
    # `kinqhi tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.kinqhi/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.kinqhi/.env (or your shell) and never in config.yaml.
            ...
            # ~/.kinqhi/bin/ on first use.  When False you must install
            ...
            # `kinqhi secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'kinqhi model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Kinqhi auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Kinqhi, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KINQHI_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #kinqhi)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: kinqhi-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'kinqhi' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Kinqhi API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Kinqhi API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Kinqhi API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # KINQHI_TOOL_PROGRESS and KINQHI_TOOL_PROGRESS_MODE are deprecated —
    ...
    "KINQHI_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kinqhi.config`` entries, then checks
    ...
    try:
        ...
        # should never break `kinqhi update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Kinqhi versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kinqhi_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'kinqhi setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Kinqhi won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'kinqhi doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("KINQHI_HOME", "~/.kinqhi")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("KINQHI_TOOL_PROGRESS")
            old_mode = get_env_value("KINQHI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from KINQHI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from KINQHI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KINQHI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KINQHI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$KINQHI_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `kinqhi plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `kinqhi curator status` has no
    ...
    #   3. Creates `~/.kinqhi/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_kinqhi_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_kinqhi_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from kinqhi_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: kinqhi config set <key> <value>")
    ...
    # metadata.kinqhi.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _kinqhi_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kinqhi/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kinqhi/config.yaml.
    ...
    (which change ``KINQHI_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/kinqhi/config.yaml invalidates the
...
#   openai-codex (OAuth — kinqhi auth) — OpenAI Codex
#   nous         (OAuth — kinqhi auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.kinqhi/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kinqhi/.env.
    ...
    interactive menu render (`kinqhi tools`, `kinqhi setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `kinqhi tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Kinqhi env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kinqhi/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kinqhi/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kinqhi/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.kinqhi/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Kinqhi — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Kinqhi vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kinqhi/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Kinqhi Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from kinqhi_cli.auth import get_anthropic_key
    ...
    # Warn on stale KINQHI_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("KINQHI_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale KINQHI_MAX_ITERATIONS={_env_ghost} "
                f"(run 'kinqhi doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  kinqhi config edit     # Edit config file", Colors.DIM))
    print(color("  kinqhi config set <key> <value>", Colors.DIM))
    print(color("  kinqhi setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: kinqhi config set <key> <value>")
            print()
            print("Examples:")
            print("  kinqhi config set model anthropic/claude-sonnet-4")
            print("  kinqhi config set terminal.backend docker")
            print("  kinqhi config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'kinqhi config migrate' to add them")
        ...
        print("  kinqhi config           Show current configuration")
        print("  kinqhi config edit      Open config in editor")
        print("  kinqhi config set <key> <value>   Set a config value")
        print("  kinqhi config check     Check for missing/outdated config")
        print("  kinqhi config migrate   Update config with new options")
        print("  kinqhi config path      Show config file path")
        print("  kinqhi config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``kinqhi config`` UI so users
