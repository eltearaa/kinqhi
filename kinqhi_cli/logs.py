"""``kinqhi logs`` — view and filter Kinqhi log files.
...
Usage examples::
    ...
    kinqhi logs                    # last 50 lines of agent.log
    kinqhi logs -f                 # follow agent.log in real time
    kinqhi logs errors             # last 50 lines of errors.log
    kinqhi logs gateway -n 100    # last 100 lines of gateway.log
    kinqhi logs gui -f            # follow gui.log (dashboard/pty/ws)
    kinqhi logs desktop -f        # follow desktop.log (Electron app boot/backend)
    kinqhi logs --level WARNING    # only WARNING+ lines
    kinqhi logs --session abc123   # filter by session ID substring
    kinqhi logs --component tools  # only tool-related lines
    kinqhi logs --since 1h         # lines from the last hour
    kinqhi logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kinqhi_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Kinqhi runs — try 'kinqhi chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_kinqhi_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kinqhi_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kinqhi_home()}/logs/")
        return

    print(f"Log files in {display_kinqhi_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kinqhi chat' to generate logs)")
