...
Used by ``kinqhi kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``kinqhi_cli/goals.py`` — same aux
...
KINQHI_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("KINQHI_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Kinqhi board.
...
def _profile_author() -> str:
    """Mirror of ``kinqhi_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("KINQHI_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=KINQHI_KANBAN_SPECIFY_MAX_TOKENS,
