"""Shell completion script generation for kinqhi CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Kinqhi bash completion
# Add to ~/.bashrc:
#   eval "$(kinqhi completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.kinqhi/profiles"
...
complete -F _hermes_completion kinqhi
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef kinqhi
# Kinqhi zsh completion
# Add to ~/.zshrc:
#   eval "$(kinqhi completion zsh)"

_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kinqhi/profiles" ]]; then
        profiles+=($HOME/.kinqhi/profiles/*(N/:t))
    fi
...
{top_cmds_str}
            ...
            _describe 'kinqhi command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Kinqhi fish completion",
        "# Add to your config:",
        "#   kinqhi completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.kinqhi/profiles",
        "        for d in $HOME/.kinqhi/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c kinqhi -f -s p -l profile"
