...
Profile-aware location: ``$KINQHI_HOME/logs/dashboard-auth.log``.
...
from ``kinqhi_constants`` or other kinqhi_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$KINQHI_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``kinqhi_constants.get_kinqhi_home`` semantics: env var wins,
    else ``~/.kinqhi``. A local copy avoids an import cycle with the
    middleware which lives below ``kinqhi_cli``.
    """
    home = os.environ.get("KINQHI_HOME") or str(Path.home() / ".kinqhi")
