...
_UNSET = object()
_KINQHI_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_KINQHI_HOME_OVERRIDE", default=_UNSET
)


def set_kinqhi_home_override(path: str | Path | None) -> Token:
    """Set a context-local Kinqhi home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _KINQHI_HOME_OVERRIDE.set(value)


def reset_kinqhi_home_override(token: Token) -> None:
    """Restore the previous context-local Kinqhi home override."""
    _KINQHI_HOME_OVERRIDE.reset(token)


def get_kinqhi_home_override() -> str | None:
    """Return the active context-local Kinqhi home override, if any."""
    override = _KINQHI_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_kinqhi_home() -> Path:
    """Return the platform-native default Kinqhi home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "kinqhi"
    return Path.home() / ".kinqhi"


def get_kinqhi_home() -> Path:
    """Return the Kinqhi home directory (default: platform-native path).

    Reads KINQHI_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``KINQHI_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``KINQHI_HOME`` explicitly (see the systemd
    template in ``kinqhi_cli/gateway.py`` and the kanban dispatcher in
    ``kinqhi_cli/kanban_db.py``).  See https://github.com/NousResearch/kinqhi/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_kinqhi_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[KINQHI_HOME fallback] KINQHI_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass KINQHI_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_kinqhi_home()


def get_default_hermes_root() -> Path:
    """Return the root Kinqhi directory for profile-level operations.

    In standard deployments this is the platform-native Kinqhi home
    (``~/.kinqhi`` on POSIX, ``%LOCALAPPDATA%\\kinqhi`` on native Windows).

    In Docker or custom deployments where ``KINQHI_HOME`` points outside
    ``~/.kinqhi`` (e.g. ``/opt/data``), returns ``KINQHI_HOME`` directly
    — that IS the root.

    In profile mode where ``KINQHI_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.kinqhi/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_kinqhi_home()
    env_home = os.environ.get("KINQHI_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # KINQHI_HOME is under ~/.kinqhi (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — KINQHI_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Kinqhi is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``KINQHI_OPTIONAL_SKILLS``.
    """
    override = os.getenv("KINQHI_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kinqhi_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``KINQHI_OPTIONAL_MCPS``.
    """
    override = os.getenv("KINQHI_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kinqhi_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``KINQHI_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<KINQHI_HOME>/skills`` last-resort
    """
    override = os.getenv("KINQHI_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_kinqhi_home() / "skills"


def get_kinqhi_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Kinqhi subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to KINQHI_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to KINQHI_HOME (e.g. ``"image_cache"``).
...
def iter_hermes_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Kinqhi-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\kinqhi\\node``. POSIX installs use
    ``$KINQHI_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_kinqhi_home()
    dirs = [root / "node"]
...
def find_hermes_node_executable(command: str) -> str | None:
    """Return a Kinqhi-managed Node/npm executable path, if installed."""
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring Kinqhi-managed installs.

    This is for Kinqhi-owned subprocesses that should not be broken by a bad,
...
def with_hermes_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Kinqhi-managed Node directories prepended to PATH."""
...
def display_kinqhi_home() -> str:
    """Return a user-friendly display string for the current KINQHI_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.kinqhi``
        profile:  ``~/.kinqhi/profiles/coder``
        custom:   ``/opt/kinqhi-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.kinqhi``.  For code that needs a real ``Path``, use
    :func:`get_kinqhi_home` instead.
    """
    home = get_kinqhi_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``KINQHI_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/kinqhi/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{KINQHI_HOME}/home`` when the profile-home directory exists."""
    kinqhi_home = get_kinqhi_home_override() or (env or {}).get("KINQHI_HOME") or os.getenv("KINQHI_HOME")
    if not kinqhi_home:
        return None
    profile_home = os.path.join(kinqhi_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("KINQHI_REAL_HOME") or os.getenv("KINQHI_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Kinqhi profile HOME.

    ``KINQHI_HOME`` scopes Kinqhi state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={KINQHI_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{KINQHI_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{KINQHI_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Kinqhi' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["KINQHI_REAL_HOME"] = real_home
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under KINQHI_HOME.

    Replaces the ``get_kinqhi_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, kinqhi_logging.py, kinqhi_time.py, etc.).
    """
    return get_kinqhi_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under KINQHI_HOME."""
    return get_kinqhi_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under KINQHI_HOME."""
