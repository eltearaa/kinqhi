...
# Python-driven tool session (e.g. Kinqhi terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/kinqhi.git"
REPO_URL_HTTPS="https://github.com/NousResearch/kinqhi.git"
KINQHI_HOME="${KINQHI_HOME:-$HOME/.kinqhi}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${KINQHI_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$KINQHI_INSTALL_DIR"
...
#   code at /usr/local/lib/kinqhi, command at /usr/local/bin/kinqhi,
#   data still at /root/.kinqhi (KINQHI_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$KINQHI_HOME/.no-bundled-skills so future"
            echo "                   'kinqhi update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Kinqhi.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.kinqhi/kinqhi"
            echo "                   default (root, Linux): /usr/local/lib/kinqhi"
            echo "  --kinqhi-home PATH  Data directory (default: ~/.kinqhi, or \$KINQHI_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Kinqhi installs the code under"
            echo "  /usr/local/lib/kinqhi and links the command into"
            echo "  /usr/local/bin/kinqhi (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$KINQHI_HOME"
            echo "  (default /root/.kinqhi).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$KINQHI_HOME/kinqhi are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs kinqhi setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `kinqhi update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Kinqhi-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Kinqhi","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install kinqhi command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `kinqhi` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KINQHI_HOME/kinqhi
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KINQHI_HOME/kinqhi
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/kinqhi
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KINQHI_HOME/kinqhi — then preserve it)
#
# Always no-op when the user set --dir or $KINQHI_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in KINQHI_HOME.
    if is_termux; then
        INSTALL_DIR="$KINQHI_HOME/kinqhi"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KINQHI_HOME/kinqhi/.git" ]; then
            INSTALL_DIR="$KINQHI_HOME/kinqhi"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/kinqhi)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/kinqhi"
        ...
        # /usr/local/bin/kinqhi wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/kinqhi"
        log_info "  Data:    $KINQHI_HOME (unchanged)"
...
# Point a Kinqhi-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $KINQHI_HOME/node/bin — which is NOT on
...
# Kinqhi's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Kinqhi-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$KINQHI_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$KINQHI_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$KINQHI_HOME/node/etc/npmrc"
}

get_hermes_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/kinqhi" ]; then
        echo "$link_dir/kinqhi"
    else
        echo "kinqhi"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://kinqhi.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Kinqhi owns its own uv at $KINQHI_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (kinqhi_cli/managed_uv.py) looks in the same
    # place, so install.sh and `kinqhi update` stay in sync.
    local _managed_uv="$KINQHI_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $KINQHI_HOME/bin ..."
    mkdir -p "$KINQHI_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/kinqhi-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/kinqhi-uv-installer.$$.sh")"
    ...
    # directly into $KINQHI_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$KINQHI_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Kinqhi-
...
check_node() {
    ...
    # Repair pre-existing Kinqhi-managed installs where `npm install -g` lands
    ...
    # Prefer a Kinqhi-managed Node from a previous run over a too-old system one.
    if [ -x "$KINQHI_HOME/node/bin/node" ] && node_satisfies_build "$("$KINQHI_HOME/node/bin/node" --version)"; then
        export PATH="$KINQHI_HOME/node/bin:$PATH"
        log_success "Node.js $("$KINQHI_HOME/node/bin/node" --version) found (Kinqhi-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Kinqhi-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.kinqhi/node/..."
    ...
    # Place into ~/.kinqhi/node/ and symlink binaries into the same bin dir
    # the kinqhi command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$KINQHI_HOME/node"
    mkdir -p "$KINQHI_HOME"
    mv "$extracted_dir" "$KINQHI_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$KINQHI_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$KINQHI_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$KINQHI_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$KINQHI_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.kinqhi/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Kinqhi install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Kinqhi itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `kinqhi update` path
                # (#4735).
                ...
                local stash_name
                stash_name="kinqhi-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Kinqhi behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up kinqhi command..."

    if [ "$USE_VENV" = true ]; then
        KINQHI_BIN="$INSTALL_DIR/venv/bin/kinqhi"
    else
        KINQHI_BIN="$(which kinqhi 2>/dev/null || echo "")"
        if [ -z "$KINQHI_BIN" ]; then
            log_warn "kinqhi not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$KINQHI_BIN" ]; then
        log_warn "kinqhi entry point not found at $KINQHI_BIN"
    ...
    # Create a user-facing shim for the kinqhi command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $KINQHI_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$KINQHI_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/kinqhi"
    cat > "$command_link_dir/kinqhi" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KINQHI_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/kinqhi"
    log_success "Installed kinqhi launcher → $command_link_display_dir/kinqhi"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "kinqhi command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v kinqhi' \
                >/dev/null 2>&1; then
        ...
        log_info "kinqhi not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Kinqhi — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Kinqhi — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Kinqhi — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so kinqhi works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.kinqhi directory structure (config at top level, code in subdir)
    mkdir -p "$KINQHI_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.kinqhi/.env (top level, easy to find)
    if [ ! -f "$KINQHI_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$KINQHI_HOME/.env"
            log_success "Created ~/.kinqhi/.env from template"
        else
            touch "$KINQHI_HOME/.env"
            log_success "Created ~/.kinqhi/.env"
        fi
    else
        log_info "~/.kinqhi/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$KINQHI_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.kinqhi/config.yaml (top level, easy to find)
    if [ ! -f "$KINQHI_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$KINQHI_HOME/config.yaml"
            log_success "Created ~/.kinqhi/config.yaml from template"
        fi
    else
        log_info "~/.kinqhi/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$KINQHI_HOME/SOUL.md" ]; then
        cat > "$KINQHI_HOME/SOUL.md" << 'SOUL_EOF'
# Kinqhi Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Kinqhi communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.kinqhi/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.kinqhi/"

    # Seed bundled skills into ~/.kinqhi/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `kinqhi update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'kinqhi update'." \
            > "$KINQHI_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $KINQHI_HOME/.no-bundled-skills"
        log_info "  Future 'kinqhi update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.kinqhi/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.kinqhi/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$KINQHI_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$KINQHI_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.kinqhi/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$KINQHI_HOME/.env"
    ...
    {
        echo ""
        echo "# Kinqhi browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Kinqhi will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (kinqhi --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `kinqhi update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'kinqhi setup' after install."
        return 0
    fi
    ...
    # Run kinqhi setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m kinqhi_cli.main setup < /dev/tty
    else
        python -m kinqhi_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Kinqhi to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$KINQHI_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'kinqhi whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                KINQHI_CMD="$(get_hermes_command_path)"
                $KINQHI_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'kinqhi whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'kinqhi gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $KINQHI_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $KINQHI_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: kinqhi gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: kinqhi gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $KINQHI_CMD gateway > "$KINQHI_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.kinqhi/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: kinqhi gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: kinqhi gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $KINQHI_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $KINQHI_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $KINQHI_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}kinqhi${NC}              Start chatting"
    echo -e "   ${GREEN}kinqhi setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}kinqhi config${NC}       View/edit configuration"
    echo -e "   ${GREEN}kinqhi config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}kinqhi gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}kinqhi update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'kinqhi' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'kinqhi' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'kinqhi' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$KINQHI_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$KINQHI_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$KINQHI_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$KINQHI_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$KINQHI_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Kinqhi for pip install"
    ...
    KINQHI_CMD="$(command -v kinqhi 2>/dev/null || echo "")"
    if [ -n "$KINQHI_CMD" ]; then
        log_info "Running kinqhi setup..."
        "$KINQHI_CMD" setup
    else
        log_warn "kinqhi command not found on PATH"
        log_info "Try: python -m kinqhi_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Kinqhi` rename
...
# _purge_electron_build_cache() used by `kinqhi desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Kinqhi-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `kinqhi desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Kinqhi" ]; then
            app="$desktop_dir/release/linux-unpacked/Kinqhi"
        elif [ -x "$desktop_dir/release/linux-unpacked/kinqhi" ]; then
            app="$desktop_dir/release/linux-unpacked/kinqhi"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Kinqhi.app" \
            "$desktop_dir/release/mac/Kinqhi.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Kinqhi is damaged and can't be opened").
    ...
    # checkout stays clean for the next `kinqhi update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Kinqhi-managed Node
            # provisioned during prerequisites/node-deps (at $KINQHI_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $KINQHI_HOME.
    # $KINQHI_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'kinqhi update' on this host install.
