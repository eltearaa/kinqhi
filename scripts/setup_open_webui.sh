...
# Bootstrap Open WebUI against Kinqhi's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.kinqhi/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-kinqhi.sh
...
#   KINQHI_API_PORT=8642
#   KINQHI_API_HOST=127.0.0.1
#   KINQHI_API_MODEL_NAME='Kinqhi'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Kinqhi WebUI}"
...
KINQHI_ENV_FILE="${KINQHI_ENV_FILE:-$HOME/.kinqhi/.env}"
KINQHI_API_PORT="${KINQHI_API_PORT:-8642}"
KINQHI_API_HOST="${KINQHI_API_HOST:-127.0.0.1}"
KINQHI_API_CONNECT_HOST="${KINQHI_API_CONNECT_HOST:-127.0.0.1}"
KINQHI_API_MODEL_NAME="${KINQHI_API_MODEL_NAME:-Kinqhi}"
KINQHI_API_BASE_URL="http://${KINQHI_API_CONNECT_HOST}:${KINQHI_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-kinqhi.sh"
LOG_DIR="$HOME/.kinqhi/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$KINQHI_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$KINQHI_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.kinqhi.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.kinqhi</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.kinqhi"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.kinqhi"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-kinqhi.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Kinqhi
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-kinqhi.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.kinqhi/logs/openwebui.log
StandardError=append:%h/.kinqhi/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-kinqhi.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$KINQHI_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Kinqhi API server is configured...'
  upsert_env API_SERVER_ENABLED true "$KINQHI_ENV_FILE"
  upsert_env API_SERVER_HOST "$KINQHI_API_HOST" "$KINQHI_ENV_FILE"
  upsert_env API_SERVER_PORT "$KINQHI_API_PORT" "$KINQHI_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$KINQHI_API_MODEL_NAME" "$KINQHI_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$KINQHI_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Kinqhi gateway so API server settings take effect...'
  kinqhi gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${KINQHI_API_CONNECT_HOST}:${KINQHI_API_PORT}/health" >/dev/null; then
    log 'Kinqhi API server did not answer on the first check. Trying to start gateway in the background...'
    nohup kinqhi gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${KINQHI_API_CONNECT_HOST}:${KINQHI_API_PORT}/health" >/dev/null
  ...
  log "Kinqhi API endpoint: ${KINQHI_API_BASE_URL}"
