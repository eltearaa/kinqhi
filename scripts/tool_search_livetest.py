#!/usr/bin/env python3
"""Live test harness for Kinqhi's Tool Search feature.
...
# Force-isolate the test environment BEFORE any kinqhi imports.
ORIGINAL_HOME = os.environ.get("KINQHI_HOME")
ORIGINAL_AUTH = Path.home() / ".kinqhi" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.kinqhi/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.kinqhi/.env`` so
    ...
    if real_env_file.exists():
        ...
        load_hermes_dotenv(kinqhi_home=str(Path.home() / ".kinqhi"))
    ...
    (kinqhi_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return kinqhi_home
...
def reset_module_state():
    """Drop cached modules so the new KINQHI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "kinqhi_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["KINQHI_HOME"] = str(home)
...
def main():
    ...
    # Restore original KINQHI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["KINQHI_HOME"] = ORIGINAL_HOME
    else:
