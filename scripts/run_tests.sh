#!/usr/bin/env bash
# Canonical test runner for kinqhi. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.kinqhi/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.kinqhi/kinqhi/venv"; do
...
if [ -f "$HOME/.kinqhi/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.kinqhi"
